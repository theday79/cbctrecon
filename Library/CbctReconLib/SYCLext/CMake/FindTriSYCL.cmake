...
#
# User must define TRISYCL_INCLUDE_DIR pointing to the triSYCL include directory.
#
# Latest version of this file can be found at: https://github.com/triSYCL/triSYCL
...
if(CMAKE_COMPILER_IS_GNUCXX)
  ...
  if(CMAKE_CXX_COMPILER_VERSION VERSION_LESS 5.4)
    message(FATAL_ERROR "host compiler - Not found! (gcc version must be at least 5.4)")
  else()
    ...
    # Disabling specific warnings warning: ignoring attributes on template argument
...
elseif("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang")
  ...
  if(${CMAKE_CXX_COMPILER_VERSION} VERSION_LESS 3.9)
    message(FATAL_ERROR "host compiler - Not found! (clang version must be at least 3.9)")
  else()
    ...
    # Disabling specific warnings warning: 'const' type qualifier on return type has no
    # effect
    add_compile_options("-Wno-ignored-qualifiers")
    # warning: comparison between signed and unsigned integer expressions
    add_compile_options("-Wno-sign-compare")
    ...
    # warning: instantiation of variable '<templated id>' required here, but no definition
    # is available
    add_compile_options("-Wno-undefined-var-template")

  endif()
elseif("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
  # Change to /std:c++latest once Boost::funtional is fixed (1.63.0 with toolset v141 not
  # working)
  add_compile_options("/std:c++14")
  # Replace default Warning Level 3 with 4 (/Wall is pretty-much useless on MSVC system
  # headers are plagued with warnings)
  string(
    REGEX
    REPLACE "/W[0-9]"
            "/W4"
  ...
  # Disabling (default) Warning Level 3 output warning C4996: Call to '<algorithm name>'
  # with parameters that may be unsafe - this call relies on the caller to check that the
  ...
  # Disabling Warning Level 4 output warning C4100: '<param>': unreferenced formal
  # parameter
  ...
  message(WARNING "host compiler - Not found! (triSYCL supports GCC, Clang and MSVC)")
endif()
...
# triSYCL definitions
set(CL_SYCL_LANGUAGE_VERSION
    121
    CACHE VERSION "Host language version to be used by triSYCL (default is: 121)")
set(TRISYCL_CL_LANGUAGE_VERSION
    121
    CACHE VERSION "Device language version to be used by triSYCL (default is: 121)")
...
if(TRISYCL_OPENCL)
  ...
  if(UNIX)
    set(BOOST_COMPUTE_INCPATH
        /usr/include/compute
        CACHE PATH "Path to Boost.Compute headers (default is: /usr/include/compute)")
  endif(UNIX)
endif()
...
if(TRISYCL_DEBUG
   OR TRISYCL_DEBUG_STRUCTORS
   OR TRISYCL_TRACE_KERNEL)
  set(LOG_NEEDED ON)
else()
  set(LOG_NEEDED OFF)
endif()
...
if(NOT TRISYCL_INCLUDE_DIR)
  message(
    FATAL_ERROR "triSYCL include directory - Not found! (please set TRISYCL_INCLUDE_DIR")
else()
...
function(add_sycl_to_target targetName)
  ...
  target_link_libraries(
    ${targetName}
    PUBLIC $<$<BOOL:${TRISYCL_OPENCL}>:${OpenCL_LIBRARIES}>
           Threads::Threads
  ...
  target_compile_definitions(
    ${targetName}
    PUBLIC $<$<BOOL:${TRISYCL_NO_ASYNC}>:TRISYCL_NO_ASYNC>
  ...
  target_compile_options(
    ${targetName} PUBLIC ${TRISYCL_COMPILE_OPTIONS}
                         $<$<BOOL:${TRISYCL_OPENMP}>:${OpenMP_CXX_FLAGS}>)

  if(${TRISYCL_OPENMP} AND (NOT WIN32))
    # Does not support generator expressions
    set_target_properties(${targetName} PROPERTIES LINK_FLAGS ${OpenMP_CXX_FLAGS})
