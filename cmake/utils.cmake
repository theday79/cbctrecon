# Returns the relative path of the current source dir against the source tree root.
#
...
# Gets the headers directory equivalent to the current source subtree.
#
# The library is organized in two directories:
#
# * Headers: Root at <source-dir>/include/siplasplas/, one directory per module/library
#   target.
#
# * Sources: The source tree, with root at <source-dir>/src/, one subdirectory per
#   module/library. This is where CMakeLists.txt files are.
#
# Given a library target organised in that way, the function returns the path to the
# headers folder of the target/s being processed in this sourcetree location..
#
...
# Unzips a library list so libraries tagged as 'debug', 'optimized', or 'general' are
# arranged into isolated lists.
#
# When passing libraries to commands like target_link_libraries() CMake tags them so the
# linker/user may know which of these libraries should be used in several compilation
# modes.
#
# Script took from
# https://software.lanl.gov/MeshTools/trac/browser/cmake/modules/ParseLibraryList.cmake
function(parse_library_list LIBRARIES)
  foreach(item ${LIBRARIES})
    if(${item} MATCHES debug
       OR ${item} MATCHES optimized
       OR ${item} MATCHES general)
...
# Gets the set of target include directories, recusivelly scanning dependencies and
# checking kind of target
...
function(
  ...
  if(TINYREFL_UTILS_DEBUG_GETTARGETDEPENDENCIES)
    message(
      STATUS
        "${padding}[${stack_length}] ${TARGET} ${notatarget}(from '${ROOT_TARGET}': ${stack_string})"
    )
  endif()
  ...
  if(("${TARGET}" STREQUAL "${ROOT_TARGET}") AND (stack_length GREATER 0))
    ...
    message(
      FATAL_ERROR
        "Dependency cycle detected on ${ROOT_TARGET} target. Full dependency chain: ${stack_string}"
    )
  endif()
...
function(add_prebuild_command)
  ...
  # Here we force the command target to be the uppermost dependency of the TARGET, so the
  # command is run only after building all other TARGET dependencies
...
# Copies dll dependencies of a target to its runtime directory.
#
# Recursivelly scans for dll dependencies, so all needed dlls are placed within the target
# output binary (usually an executable). This helps to run/debug executables directly in
# the buildtree, without installing them.
#
...
function(clangxx_stdlib_includes stdlib INCLUDES)
  ...
  foreach(compiler_exec ${compilers})
    # We want DRLParser to work even if MinGW is not available on Windows, but note that
    # would add some innaccuracies when parsing with libclang (Missing standard library
    # headers, etc)
    ...
    # Since GNU has localization in most of their tools (GCC included) the diagnostics are
    # translated to the different user languages. We have to handle the different
    # languages...
    #
    # The fun thing of this is that GNU translation to Spanish seems to be done by a bad
    # Google Translate bot or something
