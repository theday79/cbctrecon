...
# CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS will be installed with install(PROGRAMS ...) into bin
# for WIN32 and lib for non-WIN32. If CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_SKIP is set to
# TRUE before including this file, then the INSTALL command is not called. The user can
# use the variable CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS to use a custom install command and
# install them however they want. If it is the MSVC compiler, then the microsoft run time
# libraries will be found and automatically added to the
# CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS, and installed. If CMAKE_INSTALL_DEBUG_LIBRARIES is
# set and it is the MSVC compiler, then the debug libraries are installed when available.
# If CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY is set then only the debug libraries are installed
# when both debug and release are available. If CMAKE_INSTALL_MFC_LIBRARIES is set then
# the MFC run time libraries are installed as well as the CRT run time libraries. If
# CMAKE_INSTALL_SYSTEM_RUNTIME_DESTINATION is set then the libraries are installed to that
# directory rather than the default. If CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS is
# NOT set, then this file warns about required files that do not exist. You can set this
# variable to ON before including this file to avoid the warning. For example, the Visual
# Studio Express editions do not include the redistributable files, so if you include this
...
if(MSVC)
  ...
  if(MSVC80)
    ...
    # Install the manifest that allows DLLs to be loaded from the directory containing the
    # executable.
  ...
  if(MSVC90)
    ...
    find_path(
      ...
      PATHS "${msvc_install_dir}/../../VC/redist"
            "${msvc_express_install_dir}/../../VC/redist" "${base_dir}/VC/redist")
  ...
  macro(MSVCRT_FILES_FOR_VERSION version)
    ...
    get_filename_component(
      msvc_install_dir
      "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\VisualStudio\\${v}.0;InstallDir]"
      ABSOLUTE)
    find_path(
      MSVC${v}_REDIST_DIR
      NAMES ${CMAKE_MSVC_ARCH}/Microsoft.VC${v}0.CRT
      PATHS "${msvc_install_dir}/../../VC/redist"
            "${base_dir}/VC/redist"
    ...
    if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
      set(__install__libs "${MSVC${v}_CRT_DIR}/msvcp${v}0.dll"
    ...
    if(CMAKE_INSTALL_DEBUG_LIBRARIES)
      ...
      set(__install__libs ${__install__libs} "${MSVC${v}_CRT_DIR}/msvcp${v}0d.dll"
  ...
  if(CMAKE_INSTALL_MFC_LIBRARIES)
    ...
    if(MSVC80)
      ...
      # Install the manifest that allows DLLs to be loaded from the directory containing
      # the executable.
      if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
        set(__install__libs
            ${__install__libs}
      ...
      set(MSVC80_MFCLOC_DIR
          "${MSVC80_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC80.MFCLOC")
      # Install the manifest that allows DLLs to be loaded from the directory containing
      # the executable.
      set(__install__libs
          ${__install__libs}
    ...
    if(MSVC90)
      ...
      set(MSVC90_MFCLOC_DIR
          "${MSVC90_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC90.MFCLOC")
      # Install the manifest that allows DLLs to be loaded from the directory containing
      # the executable.
      set(__install__libs
          ${__install__libs}
    ...
    macro(MFC_FILES_FOR_VERSION version)
      ...
      set(MSVC${v}_MFCLOC_DIR
          "${MSVC${v}_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC${v}0.MFCLOC")
      set(__install__libs
          ${__install__libs}
  ...
  # MSVC 8 was the first version with OpenMP Furthermore, there is no debug version of
  # this
  if(CMAKE_INSTALL_OPENMP_LIBRARIES)
    macro(OPENMP_FILES_FOR_VERSION version_a version_b)
      set(va "${version_a}")
      set(vb "${version_b}")
      set(MSVC${va}_OPENMP_DIR
          "${MSVC${va}_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC${vb}.OPENMP")

      if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
        set(__install__libs ${__install__libs} "${MSVC${va}_OPENMP_DIR}/vcomp${vb}.dll")
      endif()
    endmacro()
  ...
  foreach(lib ${__install__libs})
    if(EXISTS ${lib})
      set(CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS ${CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS} ${lib})
      message(STATUS ">> ${lib}")
    else()
      if(NOT CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS)
        message(WARNING "system runtime library file does not exist: '${lib}'")
        # This warning indicates an incomplete Visual Studio installation or a bug
        # somewhere above here in this file. If you would like to avoid this warning, fix
        # the real problem, or set CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS before
        # including this file.
      endif()
    endif()
  endforeach()
...
if(WATCOM)
  ...
  if(WATCOM17)
    set(__install__libs ${CompilerPath}/clbr17.dll ${CompilerPath}/mt7r17.dll
                        ${CompilerPath}/plbr17.dll)
  endif()
  if(WATCOM18)
    set(__install__libs ${CompilerPath}/clbr18.dll ${CompilerPath}/mt7r18.dll
                        ${CompilerPath}/plbr18.dll)
  endif()
  if(WATCOM19)
    set(__install__libs ${CompilerPath}/clbr19.dll ${CompilerPath}/mt7r19.dll
                        ${CompilerPath}/plbr19.dll)
  endif()
  foreach(lib ${__install__libs})
    if(EXISTS ${lib})
      set(CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS ${CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS} ${lib})
      ...
      if(NOT CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS)
        message(WARNING "system runtime library file does not exist: '${lib}'")
        # This warning indicates an incomplete Watcom installation or a bug somewhere
        # above here in this file. If you would like to avoid this warning, fix the real
        # problem, or set CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS before including
        # this file.
      endif()
    endif()
  endforeach()
