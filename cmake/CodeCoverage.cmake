...
#
# From this point, there are two primary methods for adding instrumentation to targets: 1
# - A blanket instrumentation by calling `add_code_coverage()`, where all targets in that
# directory and all subdirectories are automatically instrumented. 2 - Per-target
# instrumentation by calling `target_code_coverage(<TARGET_NAME>)`, where the target is
# given and thus only that target is instrumented. This applies to both libraries and
# executables.
#
# To add coverage targets, such as calling `make ccov` to generate the actual coverage
# information for perusal or consumption, call `target_code_coverage(<TARGET_NAME>)` on an
# *executable* target.
#
...
# ~~~
#
# Example 2: Target instrumented, but with regex pattern of files to be excluded from
# report
#
# ~~~
...
# Adds code coverage instrumentation to a library, or instrumentation/targets for an
# executable target.
# ~~~
    ...
    if(target_type STREQUAL "EXECUTABLE")
      if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang"
        ...
        add_custom_target(
          ...
          COMMAND
            ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> ${SO_OBJECTS}
            -instr-profile=${TARGET_NAME}.profdata -show-line-counts-or-regions
            -output-dir=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${TARGET_NAME} -format="html"
            ${EXCLUDE_REGEX}
          DEPENDS ccov-processing-${TARGET_NAME})
      ...
      elseif(CMAKE_COMPILER_IS_GNUCXX)
        ...
        add_custom_target(
          ...
          COMMAND ${GENHTML_PATH} -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${TARGET_NAME}
                  ${COVERAGE_INFO}
          DEPENDS ccov-preprocessing ${TARGET_NAME})
      endif()
...
# Adds code coverage instrumentation to all targets in the current directory and any
# subdirectories. To add coverage instrumentation to only specific targets, use
# `target_code_coverage`.
...
# Adds the 'ccov-all' type targets that calls all targets added via `target_code_coverage`
# with the `ALL` parameter, but merges all the coverage data from them into a single large
# report  instead of the numerous smaller reports.
# ~~~
# Optional:
...
function(add_code_coverage_all_targets)
  ...
  if(CODE_COVERAGE)
    if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}"
      ...
      # Export coverage information so continuous integration tools (e.g. Jenkins) can
      # consume it
      add_custom_target(
        ccov-all-export
        COMMAND
