# FFTW can be compiled and subsequently linked against various data types. There is a
# single set of include files, and then muttiple libraries, One for each type.  I.e.
# libfftw.a-->double, libfftwf.a-->float

# The following logic belongs in the individual package mark_as_advanced(ITK_USE_FFTWD)
# option(ITK_USE_FFTWD "Use double precision FFTW if found" ON)
# mark_as_advanced(ITK_USE_FFTWF) option(ITK_USE_FFTWF "Use single precision FFTW if
# found" ON)

# FFTW can be included from Intel MKL library (static). Both `ITK_USE_FFTWD` and
# `ITK_USE_FFTWF` will be turned ON.
#
# To use the MKL implementation, set `ITK_USE_MKL` to ON and set `MKLROOT` to the
# installation directory of the MKL library on your system and point to the `mkl`
# subdirectory: ** On Linux, the default install directory is:
# /opt/intel/compilers_and_libraries/linux/ ** On Windows, it is: `C:\Program Files
# (x86)\IntelSWTools\compilers_and_libraries_2018.2.185\windows\mkl` ** On Mac, it is:
# `/opt/intel/compilers_and_libraries/mac/mkl`
#
# '''Disabled''': If the compiler supports it, `ITK_USE_TBB_WITH_MKL` is automatically set
# to ON and MKL is linked against `TBB`. Otherwise it is linked against `mkl_sequential`.
#
# Note: if the environment variable `MKLROOT` is set, its value will be used to set the
# initial value of the CMake variable `MKLROOT` (see
...
if(ITK_USE_FFTWD OR ITK_USE_FFTWF)
  ...
  elseif(ITK_USE_MKL)
    ...
    # result in conflicts if `Module_ITKTBB` is ON. if ("${CMAKE_CXX_COMPILER_ID}"
    # STREQUAL "Clang") # See MKL configuration help page: #
    # https://software.intel.com/en-us/articles/intel-mkl-link-line-advisor message("TBB
    # threading option not available with Clang compiler.") set(ITK_USE_TBB_WITH_MKL OFF
    # CACHE BOOL "Use TBB threading in MKL" FORCE) else() option(ITK_USE_TBB_WITH_MKL "Use
    # TBB threading in MKL" ON) mark_as_advanced(ITK_USE_TBB_WITH_MKL) endif()
    ...
    # if(ITK_USE_TBB_WITH_MKL) list(APPEND MKL_EXTRA_LIBRARIES mkl_tbb_thread) else()
    # list(APPEND MKL_EXTRA_LIBRARIES mkl_sequential) endif()
    ...
    macro(FFTWD_LIB_END)
      if(${CMAKE_SYSTEM_NAME} MATCHES "Linux")
        list(
          ...
          -lm
          -ldl)
        # Force to sequential for now. if(ITK_USE_TBB_WITH_MKL) list(APPEND FFTWD_LIB
        # -ltbb -lstdc++) endif()
      endif()
    endmacro()

    # Because of circular dependencies between the libraries, we need to use --start-group
    # and --end-group on UNIX.
    fftwd_lib_start()
    ...
    if(ITK_USE_FFTWD)
      ...
      find_library(
        FFTWD_THREADS_LIB fftw3_threads ${FFTW_LIB_SEARCHPATH}) # Double Precision Lib
    ...
    if(ITK_USE_FFTWF)
      ...
      find_library(
        FFTWF_THREADS_LIB fftw3f_threads ${FFTW_LIB_SEARCHPATH}) # Single Precision Lib
                                                                 # only if compiled with
                                                                 # threads support
