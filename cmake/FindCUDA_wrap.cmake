...
elseif(${CMAKE_MAJOR_VERSION}.${CMAKE_MINOR_VERSION} VERSION_LESS 2.8)
  ...
  # GCS 2011.03.16 Make nvcc less whiny
  if(CMAKE_COMPILER_IS_GNUCC)
    set(CUDA_PROPAGATE_HOST_FLAGS CACHE BOOL OFF)
  endif()

  # GCS 2012-05-11:  We need to propagate cxx flags to nvcc, but the flag -ftest-coverage
  # causes nvcc to barf, so exclude that one
  if(CMAKE_COMPILER_IS_GNUCC)
    string(
      REPLACE "-ftest-coverage"
              ""
              TMP
              "${CMAKE_CXX_FLAGS}")
    string(
      REPLACE "-ftemplate-depth-50"
              ""
              TMP
              "${TMP}")
    string(
...
# JAS 08.25.2010 Check to make sure nvcc has gcc-4.3 for compiling. This script will
# modify CUDA_NVCC_FLAGS if system default is not gcc-4.3
include(nvcc-check)
...
if(CUDA_FOUND)
  try_run(
    ...
    CMAKE_FLAGS -DINCLUDE_DIRECTORIES:STRING=${CUDA_TOOLKIT_INCLUDE}
                -DLINK_LIBRARIES:STRING=${CUDA_CUDART_LIBRARY}
    COMPILE_OUTPUT_VARIABLE COMPILE_OUTPUT_VAR
    RUN_OUTPUT_VARIABLE RUN_OUTPUT_VAR)
  # COMPILE_RESULT_VAR is TRUE when compile succeeds RUN_RESULT_VAR is zero when a GPU is
  # found
  if(COMPILE_RESULT_VAR AND NOT RUN_RESULT_VAR)
    set(CUDA_HAVE_GPU
        TRUE
        CACHE BOOL "Whether CUDA-capable GPU is present")
  else()
    set(CUDA_HAVE_GPU
        FALSE
        CACHE BOOL "Whether CUDA-capable GPU is present")
