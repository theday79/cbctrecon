...
macro(
  ...
  set_target_properties(
    ${TARGET_NAME}
    PROPERTIES ARCHIVE_OUTPUT_DIRECTORY "${PLM_BINARY_DIR}"
  ...
  if(NOT
     ...
     STREQUAL
     "")
    set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS ${TARGET_LDFLAGS})
  endif()
...
macro(PLM_ADD_GPU_PLUGIN_LIBRARY TARGET_NAME TARGET_SRC)
  ...
  # Set output directory.  No PUBLIC_HEADER directory is needed, because they don't have a
  # public API.
  set_target_properties(
    ${TARGET_NAME}
    PROPERTIES ARCHIVE_OUTPUT_DIRECTORY "${PLM_BINARY_DIR}"
...
macro(PLM_ADD_OPENCL_FILE SRCS CL_FILE)
  # I don't yet know how to bundle the .cl file within the executable. Therefore, copy the
  # .cl into binary directory.
  set(${SRCS} ${${SRCS}} "${PLM_BINARY_DIR}/${CL_FILE}")
  add_custom_command(
    OUTPUT "${PLM_BINARY_DIR}/${CL_FILE}"
    COMMAND ${CMAKE_COMMAND} "-E" "copy" "${CMAKE_CURRENT_SOURCE_DIR}/${CL_FILE}"
...
macro(PLM_SET_SSE2_FLAGS)
  foreach(SRC ${ARGN})
    # JAS 08.19.2010 - Unfortunately, this doesn't work. SET_PROPERTY( SOURCE bspline.c
    # APPEND PROPERTY COMPILE_DEFINITIONS ${SSE2_FLAGS} ) So, we ask CMake more forcefully
    # to add additional compile flags
