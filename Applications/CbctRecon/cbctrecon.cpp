...
: QMainWindow(parent, flags)
{
    ...
    //m_iMedianSize = 3;
    //m_arrYKImage = nullptr; vectors are good at commiting suicide
    ...
    std::cout << "Current Default Dir: " << m_strPathDirDefault.toLocal8Bit().constData() << std::endl;
	QString tmp_folder("tmp");
    init_DlgRegistration(tmp_folder);//to Setup plastimatch folder. this is useful if registration will be only done

    //shell test
    //QString strCurFolder = "H:\\lib\\rtk\\NightlyBUILD64\\bin\\Release";
    //QString strCommand = QString("explorer %1").arg(strCurFolder);
    //cout << strCommand.toLocal8Bit().constData() << std::endl;
    //::system(strCommand.toLocal8Bit().constData());
    //::system("rtkfdk");

    //	if (QProcess::execute(QString("H:\\lib\\rtk\\NightlyBUILD64\\bin\\Release\\rtkfdk")) < 0)
    //	qDebug() << "Failed to run";
    m_strPathDirDefault = R"(D:\Program_data\01_20140827_CBCT_All\04_patient_phan_pelvis_M\IMAGES\img_1.3.46.423632.135786.1409186054.9_M20mAs6440)";

    QString strPathCurAppDir = QDir::currentPath(); //should be same as .exe file
    std::cout << "Current app path= " << strPathCurAppDir.toLocal8Bit().constData() << std::endl;
    m_strPathDefaultConfigFile = strPathCurAppDir + "/" + "DefaultConfig.cfg";

    if (!LoadCurrentSetting(m_strPathDefaultConfigFile)) //Update GUI
    {
        std::cout << "DefaultConfig.cfg is not found in the application folder. A new one will be created" << std::endl;
...
void CbctRecon::SLT_LoadImageFloat3D() //Dose image for JPhillips
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "3D dose float file (*.mha)", nullptr, nullptr);
    ...
    //
    //FloatImageType::SizeType oldSize = reader->GetOutput()->GetBufferedRegion().GetSize();
    ...
    ////cout << "mask size = " << size1 << std::endl;

    //FloatImageType::IndexType idxStart; //always should be 0 -->want to shift center? Use Origin
    //idxStart[0] = 0;
    //idxStart[1] = 0;
    //idxStart[2] = 0;
    ...
    ////cout << "mask spacing = " << spacing1 << std::endl;
...
void CbctRecon::SLT_Load3DImage() // mha reconstructed file, from external source
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "Projection file (*.mha)", nullptr, nullptr);
...
{
    QString fileName = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "short mha file (*.mha)", nullptr, nullptr);
    ...
    using ImageCalculatorFilterType2 = itk::MinimumMaximumImageCalculator<UShortImageType>;
    ...
    std::cout << "Min and Max Values are	" << minVal2 << "	" << maxVal2 << std::endl;
...
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "NKI file (*.SCAN)", nullptr, nullptr);
    ...
    std::cout << "File conversion is done. Trying to read mha file.." << std::endl;
...
void CbctRecon::SLT_DrawProjImages()
{
    ...
    //cout << "img Size_Requested: " << imgSize[0] << "	" << imgSize[1] <<"	" << imgSize[2] << std::endl; //same
    //cout << "img Size_Buffered: " << imgSizeBuf[0] << "	" << imgSizeBuf[1] <<"	" << imgSizeBuf[2] << std::endl; //same
    //cout << "img Size_LargestPossible: " << imgSizeLargest[0] << "	" << imgSizeLargest[1] <<"	" << imgSizeLargest[2] << std::endl; //same
        ...
        if (iNumSlice == iReqSlice)
        {
            ...
            while (!it.IsAtEndOfSlice())
            {
                ...
                while (!it.IsAtEndOfLine())
                {
                    ...
                    m_dspYKImgProj->m_pData[iNumWidth + width*iNumHeight] = static_cast<unsigned short>((tmpVal - m_fProjImgValueMin)*m_multiplyFactor);
...
void CbctRecon::SLT_MakeElektaXML()
{
    ...
    QString filePath_ImageDBF = QFileDialog::getOpenFileName(this, "SelectIMAGE.DBF file", m_strPathDirDefault, "Elekta DB file (*.dbf)", nullptr, nullptr);

    if (filePath_ImageDBF.length() < 2) {
        return;
}

    QString filePath_FrameDBF = QFileDialog::getOpenFileName(this, "Select FRAME.DBF file", m_strPathDirDefault, "Elekta DB file (*.dbf)", nullptr, nullptr);
    ...
    std::cout << "Generated ElektaXML path: " << genFilePath.toLocal8Bit().constData() << std::endl;
}

QString CbctRecon::MakeElektaXML(const QString& filePath_ImageDBF, const QString& filePath_FrameDBF, const QString& DICOM_UID)
{
...
QString CbctRecon::CorrectSingleFile(const char* filePath)
{
    ...
    else if (!bDarkCorrApply && bGainCorrApply)
    {
        ...
        else
        {
            ...
            MeanVal = sum / static_cast<double>(DEFAULT_ELEKTA_PROJ_WIDTH*DEFAULT_ELEKTA_PROJ_HEIGHT);

            for (int i = 0; i < DEFAULT_ELEKTA_PROJ_WIDTH * DEFAULT_ELEKTA_PROJ_HEIGHT; i++) {
                if (m_pImgGain->m_pData[i] == 0) {
                    corrImg.m_pData[i] = rawImg.m_pData[i];
                } else {
                    corrImg.m_pData[i] = static_cast<unsigned short>(static_cast<double>(rawImg.m_pData[i]) / static_cast<double>(m_pImgGain->m_pData[i])*MeanVal);
}
            }
        }
    ...
    else if (bDarkCorrApply && bGainCorrApply)
    {
        ...
        else //if not raw image
        {
            ...
            for (int i = 0; i < DEFAULT_ELEKTA_PROJ_WIDTH * DEFAULT_ELEKTA_PROJ_HEIGHT; i++)
            {
                denom = static_cast<double>(m_pImgGain->m_pData[i] - m_pImgOffset->m_pData[i]);
        ...
        else
        {
            ...
            for (int i = 0; i < DEFAULT_ELEKTA_PROJ_WIDTH * DEFAULT_ELEKTA_PROJ_HEIGHT; i++) {
                if (m_pImgGain->m_pData[i] == 0) {
                    ...
                    corrImg.m_pData[i] = static_cast<unsigned short>(static_cast<double>(pYKRawImg->m_pData[i]) / static_cast<double>(m_pImgGain->m_pData[i])*MeanVal);
}
            }
...
void CbctRecon::SLT_DrawReconImage()
{
    ...
    using ExtractFilterType = itk::ExtractImageFilter<UShortImageType, UShortImage2DType>;
    ...
    //UShortImageType::Pointer cloned3DImg = UShortImageType::New();
    ...
    //cout <<"is InPlace going in extractFilter?  " << extractFilter->GetInPlace() << std::endl;
    //cout << "InPlace Possible? " << extractFilter->CanRunInPlace() << std::endl;
    extractFilter->SetDirectionCollapseToSubmatrix();
    //cout <<"is InPlace still going?  " << extractFilter->GetInPlace() << std::endl;

    //return;


    //memory check:
    //UShortImageType::RegionType crntRegion3D = m_spReconImg->GetBufferedRegion();
    UShortImageType::RegionType crntRegion3D = clonedImage->GetBufferedRegion();

    crntRegion3D = clonedImage->GetBufferedRegion();
    //cout << crntRegion3D.GetSize()[2] << std::endl; // 01


    //cout << crntRegion3D.size[0] << std::endl;
    //cout << crntRegion3D.size[1] << std::endl;
    //UShortImageType::RegionType crntRegion3D;
    ...
    //cout << "size is " << size[0] << "	" << size[1] << "	" << size[2] << std::endl;
    ...
    //cout << "start point is " << start[0] << "	" << start[1] << "	" << start[2] << std::endl;
    ...
    /*typedef itk::ImageFileWriter<UShortImageType2D> WriterType;
    WriterType::Pointer writer = WriterType::New();
    ...
    //cout << crntRegion3D.GetSize()[2] << std::endl; // 120
    //crntRegion3D = m_spReconImg->GetBufferedRegion();
    //cout << crntRegion3D.GetSize()[2] << std::endl; // 01

    //cout <<m_dspYKReconImage->m_iWidth << "	" << m_dspYKReconImage->m_iHeight << std::endl;
...
void CbctRecon::LoadRTKGeometryFile(const char* filePath)
{
    ...
    std::cout << "Geometry data size(projection gantry angles): " << geoDataSize << std::endl;
    if (geoDataSize < 1) {
        return;
}
    ...
    {
    std::cout << "Projection gantry angle " << i << ": " << m_spFullGeometry->GetGantryAngles().at(i) << std::endl;
    }
    std::cout << "Coordination is following IEC coordinate (e.g. 190 deg = RPO, 170 LPO, 30 = LAO supposing HFS) no and kV source position (not MV gantry)" << std::endl;*/
    ...
    //cout << "AngularGapsWithNext Size: " << custGeometry->GetAngularGapsWithNext().size() << std::endl;
    //cout << "AngularGapsWithNext Mean (deg): " << vectorMean(custGeometry->GetAngularGapsWithNext())/itk::Math::pi * 180.0<< std::endl;
    std::cout << "AngularGaps Size: " << m_spFullGeometry->GetAngularGaps(m_spFullGeometry->GetSourceAngles()).size() << std::endl;

    //SLT_DoReconstruction();

    //double meanGap = vectorMean(m_spFullGeometry->GetAngularGaps())/itk::Math::pi * 180.0;
    //cout << "AngularGaps Mean (deg): " << meanGap << std::endl;
    //double angleSum = vectorSum(m_spFullGeometry->GetAngularGaps())/itk::Math::pi * 180.0;//total rot. range from first angle
    //cout << "AngularGaps Sum (deg): " << angleSum  << std::endl;

}
void CbctRecon::SLT_OpenElektaGeomFile()
{
    QString strPath = QFileDialog::getOpenFileName(this, "Select a single file to open", m_strPathDirDefault, "Geometry file (*.xml)");
...
{
    QString strPath = QFileDialog::getSaveFileName(this, "File path to save", "D:\\", "meta 3D image data (*.mha)", nullptr, nullptr); //Filename don't need to exist
...
{
	using castToCudaType = itk::CastImageFilter <FloatImageType, CUDAFloatImageType >;
	...
	if (ui.checkBox_UsePSSF->isChecked())
	{
		...
		pssf->GlobalWarningDisplayOff(); // I don't care about any of the potential warnings
	...
	if (ui.checkBox_UpdateAfterFiltering->isChecked())
	{
		...
		using CastFilterType = itk::CastImageFilter<CUDAFloatImageType, FloatImageType>;
		CastFilterType::Pointer CastFilter = CastFilterType::New();
	...
	using StreamerType = itk::StreamingImageFilter <CUDAFloatImageType, CUDAFloatImageType>;
	...
	std::cout << "Euler 3D Transformation: from RTK-procuded volume to standard DICOM coordinate" << std::endl;
	...
	using ResampleFilterType = itk::ResampleImageFilter <CUDAFloatImageType, CUDAFloatImageType>;
	ResampleFilterType::Pointer resampler = ResampleFilterType::New();
	//FloatImageType::RegionType fixedImg_Region = fixedImg->GetLargestPossibleRegion().GetSize();
	...
	using MultiplyImageFilterType = itk::MultiplyImageFilter<FloatImageType, FloatImageType, CUDAFloatImageType>;
	MultiplyImageFilterType::Pointer multiplyImageFilter = MultiplyImageFilterType::New();
	...
	multiplyImageFilter->SetConstant(65536); //calculated already
											 ...
											 //typedef unsigned short FinalPixelType;
											 //typedef itk::Image< FinalPixelType, 3 > FinalImageType;
	using CastFilterType = itk::CastImageFilter< CUDAFloatImageType, UShortImageType >;
	CastFilterType::Pointer castFilter = CastFilterType::New();
	...
	{
		using MedianFilterType = itk::MedianImageFilter<UShortImageType, UShortImageType >; // TODO(AGA): CUDA THIS!!
	...
	//cout << "TestYK" << std::endl;
	//cout << tmpReconImg->GetRequestedRegion().GetSize() << std::endl;
	//cout << "Before InPlace Off: " <<  castFilter->GetInPlace() << std::endl;	//0
	//castFilter->InPlaceOff();
	//cout << "After InPlace Off: " <<  castFilter->GetInPlace() << std::endl; //0
	...
	{
		cout << "No available output path. Should be exported later" << std::endl;
	...
	else
	{
		...
		cout << "Writing the image to: " << outputFilePath.toLocal8Bit().constData() << std::endl;
...
void CbctRecon::CudaDoReconstructionFDK(enREGI_IMAGES target)
{
	cout << "This program were not compiled with the CUDA option, please select OpenCL or CPU instead!" << std::endl;
	return;
}
#endif
...
#if USE_OPENCL_PLM
FloatImageType::Pointer PlastimatchOpenCLFDK(const FloatImageType::Pointer& spCurImg, 
	...
	std::array<float, 3> spacing, std::array<plm_long, 3> sizeOutput)
{
	...
	auto* vol = new Volume(&sizeOutput[0], &offset[0], &spacing[0], nullptr, PT_FLOAT, 1);
	cout << "Buffer vol: " << vol->pix_size * vol->npix << std::endl;
	cout << "Buffer proj: " << proj_dim[1] * proj_dim[0] * sizeof(float) << std::endl;
	...
	/* Calculate the scale */
	const float scale = static_cast<float>(sqrt(3.0) / static_cast<double>(1 + proj_dim[2])) * 10.0f;

	vector<double>::const_iterator itShiftX, itShiftY;
	...
	for (int image_num = 0; image_num < static_cast<int>(proj_dim[2]); image_num++)
	{
		...
		proj->pmat->sad = m_spCustomGeometry->GetSourceToIsocenterDistances()[image_num];

		const double sid = m_spCustomGeometry->GetSourceToDetectorDistances()[image_num];
		const double angle = m_spCustomGeometry->GetGantryAngles()[image_num];
		const double tgt[] = { 0.0, 0.0, 0.0 };
		const double cam[] = {
			tgt[0] + proj->pmat->sad * cos(angle),
			tgt[1] - proj->pmat->sad * sin(angle),
			tgt[2] };
		const double vup[] = { 0, 0, 1 }; // R U sure about that?
		const double ps[] = { spCurImg->GetSpacing()[0],  spCurImg->GetSpacing()[1] };

		proj->pmat->set(&cam[0], &tgt[0], &vup[0], sid, &proj->pmat->ic[0], &ps[0]); // , proj->dim);
		...
		opencl_set_kernel_args( // Marked as "dangerous" c-style function by cppcoreguidelines-pro-type-vararg
			&ocl_dev,
...
#if USE_OPENCL_RTK
FloatImageType::Pointer RTKOpenCLFDK(const FloatImageType::Pointer& spCurImg,
	const rtk::ThreeDCircularProjectionGeometry::Pointer& m_spCustomGeometry,
...
void CbctRecon::OpenCLDoReconstructionFDK(enREGI_IMAGES target)
{
	if (m_spProjImg3DFloat == nullptr)
	{
		cout << "processed Projection image is not ready yet" << std::endl;
		return;
	}
	...
	if (ui.checkBox_UseDDF->isChecked())
	{
		ddf->SetInput(spCurImg);
		ddf->SetGeometry(m_spCustomGeometry);
		cout << "DDF was set in pipeline" << std::endl;
	...
	if (ui.checkBox_UsePSSF->isChecked())
	{
		...
		pssf->SetGeometry(m_spCustomGeometry);
		//pssf->InPlaceOff(); //YKComments: Do not overwrite input image buffer for output
		cout << "short scan image filter success" << std::endl;
	...
	spacing[1] = ui.lineEdit_outImgSp_SI->text().toDouble();
	spacing[2] = ui.lineEdit_outImgSp_LR->text().toDouble();
	FloatImageType::Pointer targetImg = PlastimatchOpenCLFDK(spCurImg, m_spCustomGeometry, spacing, sizeOutput); //streamerBP->GetOutput();
#endif
	...
	if (fTruncCorFactor > 0.0 && target == REGISTER_COR_CBCT)
	{
		cout << "Warning! Truncation factor is " << fTruncCorFactor << ". Regardless of previous setting, this factor should not be 0 for scatter corrected CBCT. Now zero value is applied." << std::endl;
		fTruncCorFactor = 0.0;
	}

	std::array<const double, 5> fdk_options = { {
		fTruncCorFactor,
		ui.lineEdit_Ramp_HannCut->text().toDouble(),
		ui.lineEdit_Ramp_CosineCut->text().toDouble(),
		ui.lineEdit_Ramp_Hamming->text().toDouble(),
		ui.lineEdit_Ramp_HannCutY->text().toDouble()
	} };

	cout << "Starting RTK fdk" << std::endl;
	FloatImageType::Pointer targetImg = RTKOpenCLFDK(spCurImg, m_spCustomGeometry, spacing, sizeOutput, fdk_options);
#endif

	cout << "Euler 3D Transformation: from RTK-procuded volume to standard DICOM coordinate" << std::endl;
	...
	cout << "Flip filter is being applied" << std::endl;
	using FilterType = itk::FlipImageFilter<FloatImageType>;
	FilterType::Pointer flipFilter = FilterType::New();
	using FlipAxesArrayType = FilterType::FlipAxesArrayType;
	FlipAxesArrayType arrFlipAxes;
	...
	cout << "Reconstructing the image.. please wait..." << std::endl;
	UShortImageType::SizeType indexRadius{};
	indexRadius[0] = ui.lineEdit_PostMedSizeX->text().toInt(); // radius along x
	indexRadius[1] = ui.lineEdit_PostMedSizeY->text().toInt(); // radius along y
	indexRadius[2] = ui.lineEdit_PostMedSizeZ->text().toInt(); // radius along y
	if (ui.checkBox_PostMedianOn->isChecked() && (indexRadius[0] != 0 || indexRadius[1] != 0 || indexRadius[2] != 0))
	{
		...
		//this is radius. 1 --> median window 3
		cout << "Post median(3D) filtering is in the pipeline..Size(radius X Y Z) is = " << indexRadius << std::endl;
		...
		tmpReconImg = medFilter->GetOutput();
		cout << "median filtering has been done" << std::endl;
	}
	else
	{
		cout << "No post median filtering is used" << std::endl;
		castFilter->Update();
		tmpReconImg = castFilter->GetOutput();
	}
...
void CbctRecon::DoReconstructionFDK(enREGI_IMAGES target)
{
    ...
    //cout << m_spProjImg3DFloat->GetRequestedRegion().GetSize() << std::endl;
    //cout << spTmpImage->GetRequestedRegion().GetSize() << std::endl;
    //return;
...
#define SET_FELDKAMP_OPTIONS(f) \
    ...
    // Streaming depending on streaming capability of writer --> not affect the calc. speed
    using StreamerType = itk::StreamingImageFilter<FloatImageType, FloatImageType>;
    StreamerType::Pointer streamerBP = StreamerType::New();
    ...
    if (ui.checkBox_PostMedianOn->isChecked() && (indexRadius[0] != 0 || indexRadius[1] != 0 || indexRadius[2] != 0))
    {
        using MedianImageFilterType = itk::MedianImageFilter<UShortImageType, UShortImageType >;
    ...
    if (outputFilePath.length() < 2 || !outFileDir.exists())
    {
        std::cout << "No available output path. Should be exported later" << std::endl;
        ui.lineEdit_Cur3DFileName->setText("FDK-reconstructed volume");
    }
    else
    {
        ...
        std::cout << "Writing the image to: " << outputFilePath.toLocal8Bit().constData() << std::endl;
    ...
    /*FloatImageType::SizeType AfterReconSize = m_spProjImg3D->GetBufferedRegion().GetSize();
    std::cout << "AfterReconSize  " << AfterReconSize[0] << "	"
    << AfterReconSize[1] << "	"
...
void CbctRecon::SLT_SetHisDir() //Initialize all image buffer
{
    ...
    //else
    //{
    //  std::cout << "Geometry file is not ready. Find XML geometry file manually" << std::endl;
...
QString getBowtiePath(QWidget* parent, const QDir& calDir) {
	return QFileDialog::getOpenFileName(parent,
		"Find air(+bowtie) filter image for subtraction", 
		calDir.absolutePath(), "Projection (*.xim)", nullptr, nullptr);
}

std::tuple<bool, bool> CbctRecon::probeUser(const QString& guessDir) {

	QString dirPath = QFileDialog::getExistingDirectory(this, tr("Open CT DICOM Directory"), guessDir, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
	...
	if (!(dirPath.length() <= 1)) {
		...
		if (plmImg.load_native(dirPath.toLocal8Bit().constData())) {
			...
			using ImageCalculatorFilterType = itk::MinimumMaximumImageCalculator<ShortImageType>;
			...
			std::cout << "Current Min and Max Values are	" << minVal << "	" << maxVal << std::endl;

			auto outputMinVal = static_cast<USHORT_PixelType>(minVal + 1024);
			auto outputMaxVal = static_cast<USHORT_PixelType>(maxVal + 1024);

			using RescaleFilterType = itk::RescaleIntensityImageFilter<ShortImageType, UShortImageType>;
	...
	reply = QMessageBox::question(this,
		"Instant Recon?",
		"Do you want to reconstruct projections as soon as they are loaded?\n(Using the current settings)",
...
void CbctRecon::SLT_LoadSelectedProjFiles()//main loading fuction for projection images
{
    ...
    if (!IsFileNameOrderCorrect(names) && !ximIsUsed)
    {
        ...
        std::cout << "File name order was cross-checked and found to be OK!" << std::endl;
    ...
    {
        std::cout << "Critical Error! geometry file is not existing. Please retry." << std::endl;
        return;
    }

    if (geomFileInfo.fileName() == "_Frames.xml")//this is XVI XML. this code will be deleted later ,instead RTK XML will be used as a sole solution
    {
        std::cout << "XVI Geometry File was found. This will be temporarily used:" << geomPath.toLocal8Bit().constData() << std::endl;
	...
	{
		cout << "Varian XML Geometry File was found. This will be temporarily used:" << geomPath.toLocal8Bit().constData() << std::endl;
		...
		xmlWriter->SetObject(reader->GetGeometry());
		TRY_AND_EXIT_ON_ITK_EXCEPTION(xmlWriter->WriteFile());
		cout << "RTK standard Geometry XML File was created:" << "RTKgeometry.xml" << std::endl;
	...
	{
		cout << "Varian Xim XML Geometry File was found. This will be temporarily used:" << geomPath.toLocal8Bit().constData() << std::endl;
		// ADDED BY AGRAVGAARD :::::::: Thief'd from RTK/applications/ rtkvarianobigemetry.cxx
		...
		xmlWriter->SetObject(reader->GetGeometry());
		TRY_AND_EXIT_ON_ITK_EXCEPTION(xmlWriter->WriteFile());
		std::cout << "RTK standard Geometry XML File was created:" << "RTKgeometry.xml" << std::endl;
		LoadRTKGeometryFile("RTKgeometry.xml");
		std::cout << "Done!";
		// ::::::::::::::::::::::::::::LoadXMLGeometryFile(geomPath.toLocal8Bit().constData()); //will generate m_spFullGeometry
	}
    ...
    {
        std::cout << "RTK standard Geometry XML File was found:" << geomPath.toLocal8Bit().constData() << std::endl;
    ...
    {
		if (!ximIsUsed) {
			cout << "Size of geometry data and file numbers are not same! Check and retry" << std::endl;
			return;
		}
			...
			reply = QMessageBox::question(this, 
				...
				"Mismatch in number of files and Geometry information!\nHowever, Xim detected, so it may be safe to continue anyway?",
				QMessageBox::Yes | QMessageBox::No);
			if (reply == QMessageBox::Yes) {
    ...
    if (ui.Radio_ManualProjAngleGap->isChecked())
    {
        ...
        {
            std::cout << "Angle gap size is too small. Terminating the app" << std::endl;
            return;
            //bManualGap = false;
        }
    }
    ...
    if (ui.Radio_ManualProjAngleGap->isChecked())
    {
        ...
        {
            std::cout << "Index: " << *itIdx << "     " << "GantryAngle: " << m_spFullGeometry->GetGantryAngles().at(*itIdx) << std::endl;
        }
    }
    else // not manual
    {
    ...
    //for (itIdx =vSelectedIdx.begin() ; itIdx != vSelectedIdx.end() ; itIdx++ )
// #pragma omp parallel for private(itIdx) shared(m_spCustomGeometry) schedule(static)
    for (itIdx = vSelectedIdx_final.begin(); itIdx != vSelectedIdx_final.end(); itIdx++)
    {
        //9 parameters are required
        const double curSID = m_spFullGeometry->GetSourceToIsocenterDistances().at(*itIdx);
        const double curSDD = m_spFullGeometry->GetSourceToDetectorDistances().at(*itIdx);
        double curGantryAngle = m_spFullGeometry->GetGantryAngles().at(*itIdx);
		const double kVAng = curGantryAngle * 180.0 * itk::Math::one_over_pi; // 360 / 2 = 180 radians to degrees
		...
		const double curProjOffsetX = m_spFullGeometry->GetProjectionOffsetsX().at(*itIdx);
        const double curProjOffsetY = m_spFullGeometry->GetProjectionOffsetsY().at(*itIdx);

        const double curOutOfPlaneAngles = m_spFullGeometry->GetOutOfPlaneAngles().at(*itIdx);
        const double curInPlaneAngles = m_spFullGeometry->GetInPlaneAngles().at(*itIdx);

        const double curSrcOffsetX = m_spFullGeometry->GetSourceOffsetsX().at(*itIdx);
        const double curSrcOffsetY = m_spFullGeometry->GetSourceOffsetsY().at(*itIdx);
    ...
    ////YKTEMP
    ////cout << vSelectedIdx.size() << " is the number of selected index" << std::endl;
    ////cout << m_spCustomGeometry->GetGantryAngles().size() << " is the number of m_spCustomGeometry" << std::endl;
    //rtk::ThreeDCircularProjectionGeometryXMLFileWriter::Pointer xmlWriter =
    //    rtk::ThreeDCircularProjectionGeometryXMLFileWriter::New();
    ...
    //}
    //cout << countFiles << " files were copied." << std::endl;
    ////YKTEMP
	...
	using FilterReaderType = rtk::ProjectionsReader<FloatImage2DType>; //This one does a bit more than we need, but mainly statistical filters
	FilterReaderType::Pointer bowtiereader = FilterReaderType::New();          // we use is because we need the projections to be in the same unit (order of magnitude)
	...
	if (ximIsUsed) {
		...
		if (bowtiePath.length() > 1) {
			...
			answers = probeUser(guessDir.absolutePath()); // looks ugly, but allows threading
}
	}
	else {
		answers = probeUser(guessDir.absolutePath());     // ^^^
	...
	if (hisIsUsed)
	{
		...
		for (auto it = m_arrYKBufProj.begin();
			it != m_arrYKBufProj.end() && it_selected != m_vSelectedFileNames.end();
			++it, ++it_selected)
		{
	...
	// std::cout << "proj meta file was saved " << std::endl;
	//}

    std::cout << "ProjectionReader Get Spacing : " << reader->GetOutput()->GetSpacing() << std::endl;
	...
	const double theoreticalMin = 0.1541 * 1.225e-3 * sqrt(
		pow(m_spCustomGeometry->GetSourceToDetectorDistances()[0], 2) +
		pow(m_spCustomGeometry->GetSourceToDetectorDistances()[1], 2) +//mm->cm
		pow(m_spCustomGeometry->GetSourceToDetectorDistances()[2], 2)) * 0.1;

	const double correctionValue = GetMaxAndMinValueOfProjectionImage(originalMax, originalMin, reader->GetOutput()); // , theoreticalMin);
	...
	if (correctionValue != 10.0 && correctionValue != 20.0) { // 10 and 20 are error codes
		...
		originalMax = -1.0;
		originalMin = -1.0;
		if (GetMaxAndMinValueOfProjectionImage(originalMax, originalMin, reader->GetOutput()) != 20.0) {
			cout << "Reader Max, Min=" << originalMax << "	" << originalMin << std::endl;
}
	}
	...
	if (m_fResampleF != 1.0) {
		ResampleItkImage(m_spProjImg3DFloat, m_spProjImg3DFloat, m_fResampleF); // was! BROKEN AF for .his where input size != 1024 (tested with 1016) -> outputs offset -inputoffset/refactor^2 and 4 pixels too few in x and y
}
    ...
    std::cout << "YKDEBUG: Origin" << originPt[0] << ", " << originPt[1] << ", " << originPt[2] << std::endl;
    std::cout << "YKDEBUG: Size" << FloatImgSize[0] << ", " << FloatImgSize[1] << ", " << FloatImgSize[2] << std::endl;
    std::cout << "YKDEBUG: Spacing" << FloatImgSpacing[0] << ", " << FloatImgSpacing[1] << ", " << FloatImgSpacing[2] << std::endl;


    std::cout << "Raw3DProj dimension " << m_spProjImgRaw3D->GetRequestedRegion().GetSize() << std::endl;
    ...
    //cout << "adding value=" << addingValue << std::endl;
    ...
    // std::cout << "before button enabled" << std::endl;
    ui.pushButton_DoRecon->setEnabled(true);

    //cout << "before setMinMax" << std::endl;
    ui.spinBoxImgIdx->setMinimum(0);
    ...
    // std::cout << "before SetMaxAndMinValueOfProjectionImage" << std::endl;

    SetMaxAndMinValueOfProjectionImage(); //update min max projection image

    //cout << "before SLT_InitializeGraphLim" << std::endl;
    SLT_InitializeGraphLim();


    //cout << "before SLT_DrawProjImages" << std::endl;
    this->SLT_DrawProjImages();  //Update Table is called
    //SLT_DrawGraph();
    //SLT_UpdateTable();

	if (!std::get<0>(answers)) { // instaRecon
		std::cout << "FINISHED!: Loading projection files. Proceed to reconstruction" << std::endl;
	} else {
		SLT_DoReconstruction();
}
...
void CbctRecon::GetExcludeIndexByNames(const QString& outlierListPath, vector<string>& vProjFileFullPath, vector<int>& vExcludeIdx)
{
    ifstream fin;
...
double CbctRecon::GetMaxAndMinValueOfProjectionImage(double& fProjImgValueMax, double& fProjImgValueMin, const FloatImageType::Pointer& projImage) //, const double theoreticalMin)
{
    /*if (m_iImgCnt > 0)
    {
    m_fProjImgValueMax = 65535;
	...
	unsigned int n_under_thoreticalMin = 0;
	std::vector<double> v_under_theoreticalMin;
    ...
    while (!it.IsAtEnd())
    {
        while (!it.IsAtEndOfSlice())
        {
            while (!it.IsAtEndOfLine())
            {
				...
				if (theoreticalMin > tmpVal) 
					v_under_theoreticalMin.push_back(tmpVal);
	...
	if (v_under_theoreticalMin.size() > 2048) // expected max size of one column + one row
	{
		std::sort(v_under_theoreticalMin.begin(), v_under_theoreticalMin.end()); //inplace quicksort
		return v_under_theoreticalMin[2048];
	}
	return 10.0;
	*/
}
...
void CbctRecon::SLT_DataProbeProj()
{
    ...
    else
    {
        ...
        dataWidth = static_cast<int>(m_spProjImg3DFloat->GetBufferedRegion().GetSize()[0]);
        dataHeight = static_cast<int>(m_spProjImg3DFloat->GetBufferedRegion().GetSize()[1]);
    ...
    QString dspText;
    dspText = QString("(%1, %2, %3): %4").arg(dataX).arg(dataY).arg(dataZ).arg(fProbeValue, 0, 'f', 2);
...
void CbctRecon::SLT_DataProbeRecon()
{
    ...
    auto dataWidth = static_cast<int>(m_spCrntReconImg->GetBufferedRegion().GetSize()[0]);
    auto dataHeight = static_cast<int>(m_spCrntReconImg->GetBufferedRegion().GetSize()[1]);
...
void CbctRecon::SLT_UpdateTable()
{
    ...
    for (int i = 0; i < rowSize; i++)
    {
        ...
        qreal tmpVal2 = static_cast<qreal>(vProfile[i]) / fMultiPlyFactor + fMinValue;
...
bool CbctRecon::IsFileNameOrderCorrect(vector<string>& vFileNames)
{
	...
	auto index_of_nonascending = std::adjacent_find(arrNum.begin(), arrNum.end(), std::greater<>());
...
void CbctRecon::SLT_CalculateROI_Proj()
{
    ...
    if (ROI_size > 0)
    {
        ...
        strMean = QString("%1").arg((m_dspYKImgProj->m_fPixelMean_ROI / m_multiplyFactor) + m_fProjImgValueMin, 0, 'f', 2);

        QString strSD;
        strSD = QString("%1").arg(m_dspYKImgProj->m_fPixelSD_ROI / m_multiplyFactor, 0, 'f', 2);
        ui.lineEdit_ROI_mean->setText(strMean);
        ui.lineEdit_ROI_SD->setText(strSD);
    }
    else {
...
bool CbctRecon::LoadShortImageToUshort(QString& strPath, UShortImageType::Pointer& pUshortImage)
{
    ...
    std::cout << "Original Min and Max Values are	" << minVal0 << "	" << maxVal0 << std::endl;
...
void CbctRecon::SLT_LoadPlanCT_mha() //m_spRecon -->m_spRefCT
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "Plan CT file (*.mha)", nullptr, nullptr);
    ...
    //cout <<"Min and Max Values are	" << minVal << "	" << maxVal << std::endl;
...
void CbctRecon::SLT_GoForcedProbePos()// when forced probe button was clicked
{
    ...
    if (ui.radioButton_graph_proj->isChecked())
    {
        ...
        ui.labelImageRaw->x = qRound(dataX / static_cast<double>(dataWidth) * dspWidth);
        ui.labelImageRaw->y = qRound(dataY / static_cast<double>(dataHeight) * dspHeight);
    ...
    else if (ui.radioButton_graph_recon->isChecked())
    {
        ...
        if (sliceIdx < 0 || sliceIdx >= static_cast<int>(m_spCrntReconImg->GetBufferedRegion().GetSize()[2])) {
            return;
		}
        ...
        ui.labelReconImage->x = qRound(dataX / static_cast<double>(dataWidth) * dspWidth);
        ui.labelReconImage->y = qRound(dataY / static_cast<double>(dataHeight) * dspHeight);
...
void CbctRecon::PostApplyFOVDispParam()
{
    ...
    int pixPosX = qRound((physPosX - static_cast<double>(origin[0])) / static_cast<double>(spacing[0]));
    int pixPosY = qRound((physPosY - static_cast<double>(origin[1])) / static_cast<double>(spacing[1]));
    ...
    int pixTableY = qRound((physTablePosY - static_cast<double>(origin[1])) / static_cast<double>(spacing[1]));
...
void CbctRecon::CropSupInf(UShortImageType::Pointer& sp_Img, float physPosInfCut, float physPosSupCut)
{
    ...
    //
    using CropImageFilterType = itk::CropImageFilter <UShortImageType, UShortImageType>;
    ...
    std::cout << "LowPos[mm, index] = " << physPosInfCut << ", " << sizeLower[2] << std::endl;
    std::cout << "UpperPos[mm, index] = " << physPosSupCut << ", " << sizeUpper[2] << std::endl;
    ...
    /*
        ...
        {
        std::cout << "Cross-correlation error! template size is not matching ROI image even after cropping" << std::endl;
        return;
        }
    ...
    while (!it.IsAtEnd())
    {
        ...
        while (!it.IsAtEndOfSlice())
        {
            ...
            while (!it.IsAtEndOfLine())
            {
                ...
                crntPhysX = iPosX*static_cast<double>(spacing[0]) + static_cast<double>(origin[0]);
                crntPhysY = iPosY*static_cast<double>(spacing[1]) + static_cast<double>(origin[1]);
...
void CbctRecon::SLT_ExportReconUSHORT()
{
    ...
    QString strPath = QFileDialog::getSaveFileName(this, "Save Image", "", "unsigned short meta image (*.mha)", nullptr, nullptr);
    if (strPath.length() <= 1) {
        return;
}
    ...
    std::cout << "Writing is under progress...: " << strPath.toLocal8Bit().constData() << std::endl;
    writer->Update();
...
void CbctRecon::ExportReconSHORT_HU(UShortImageType::Pointer& spUsImage, QString& outputFilePath)
{
    ...
    */
    ...
    auto minVal = static_cast<double>(imageCalculatorFilter->GetMinimum());
    auto maxVal = static_cast<double>(imageCalculatorFilter->GetMaximum());
    //cout << "Min and Max Values are	" << minVal << "	" << maxVal << std::endl; //should be 0 and 4096
    //cout <<"Min and Max Values are	" << minVal << "	" << maxVal << std::endl;
    ...
    using AddImageFilterType = itk::AddImageFilter <ShortImageType, ShortImageType, ShortImageType>;
    AddImageFilterType::Pointer addImageFilter = AddImageFilterType::New();
    ...
    //cout << "Short image Min and Max Values are	" << minVal2 << "	" << maxVal2 << std::endl;
    ...
    std::cout << "Writing is under progress...: " << outputFilePath.toLocal8Bit().constData() << std::endl;
    writer->Update();
    std::cout << "Writing was successfully done" << std::endl;
...
void CbctRecon::SLT_ExportReconSHORT_HU()
{
    ...
    */
    QString strPath = QFileDialog::getSaveFileName(this, "Save Image", "", "signed short meta image (*.mha)", nullptr, nullptr);
    if (strPath.length() <= 1) {
        return;
}
    ...
    //    {
    //        std::cout << "Error! 2 mapping values are required." << std::endl;
    //        return;
    //    }
    ...
    //cout << "Writing is under progress...: " << strPath.toLocal8Bit().constData() << std::endl;
    //writer->Update();
    //cout << "Writing was successfully done" << std::endl;
    ...
    {
    std::cout << "DICOM export is under progress." << std::endl;
    ShortImageType::Pointer spTmpImgResult = spRescaleFilter->GetOutput();
    ExportDICOM_SHORT(spTmpImgResult);

    std::cout << "DICOM export is completed." << std::endl;
    }*/
...
//void CbctRecon::ExportDICOM_SHORT( ShortImageType::Pointer& sp3DshortImage ) //NOT COMPLETED YET!! Export DICOM without Source DICOM is not possible
//{
...
//
//	typedef itk::ImageSeriesWriter< ShortImageType, ShortImageType2D > SeriesWriterType;
//
...
//
//	typedef itk::ShiftScaleImageFilter< ShortImageType, ShortImageType > ShiftScaleType;
//
...
//
//	ShortImageType::SizeType outputSize = sp3DshortImage->GetBufferedRegion().GetSize();
...
//		value.str("");
//		value << "Derivation Description" << std::endl;
//
...
//
//
//		ShortImageType::SpacingType outputSpacing = sp3DshortImage->GetSpacing();
//
...
inline float BeamHardModel(const double val,
	const double a, const double b,
	const double c, const double d) 
{
	return a * pow(val, 3.0) +
		b * pow(val, 2.0) +
		c * val +
		d;
}
...
{   //           a *       x^3     +       b *       x^2     +       c *   x +       d
	return 6.0e-08 * pow(val, 3.0) - 1.0e-08 * pow(val, 2.0) - 5.0e-07 * val + 8.0e-01;
}

inline double HisBeamHardModel(const double val)
{   //             a *       x^3     +         b *       x^2     +         c *   x +         d
	return 9.321e-05 * pow(val, 3.0) - 2.609e-03 * pow(val, 2.0) + 3.374e-02 * val + 9.691e-01;
}

inline double XimBeamHardModel(const double val)
{   //           a *       x^3     +      b *       x^2     +      c *   x +   d
...
#pragma omp parallel for
	for (int i = 0; i < nPix; i++) {
		pBuffer[i] = static_cast<float>(((pBuffer[i] < 1.189) ? pBuffer[i] : pBuffer[i] * HndBeamHardModel(pBuffer[i])) + 1.47);
	}
}
void hisBeamHardening(float *pBuffer, const int nPix) {
#pragma omp parallel for
	for (int i = 0; i < nPix; i++) {
		pBuffer[i] = static_cast<float>(((pBuffer[i] < 1.189) ? pBuffer[i] : pBuffer[i] * HisBeamHardModel(pBuffer[i])));
	}
}
void ximBeamHardening(float *pBuffer, const int nPix) {
#pragma omp parallel for
	for (int i = 0; i < nPix; i++) {
		pBuffer[i] = static_cast<float>(((pBuffer[i] < 1.189) ? pBuffer[i] : pBuffer[i] * XimBeamHardModel(pBuffer[i])) - 1.47);
	}
}
void CbctRecon::DoBeamHardeningCorrection()
{
	...
	// If Hnd: // REMEMBER to change in above inlined functions, here is only for debug !!
    ...
    std::cout << "Beam hardening corrF poly curve:" << poly3_a << "	" << poly3_b << "	" << poly3_c << "	" << poly3_d << std::endl;

	float* pImgBuffer = m_spProjImg3DFloat->GetBufferPointer();
	const itk::Size<3U> pImgSize = m_spProjImg3DFloat->GetLargestPossibleRegion().GetSize();
	const unsigned int nPix = pImgSize[0] * pImgSize[1] * pImgSize[2];

	if (hisIsUsed) { // we have now helped the compiler to allow aggressive optimization
	...
	/*
#pragma omp parallel for simd private(i,corrF)
	for (unsigned int i = 0; i < nPix; i++) {
		const double crntVal = (double)pImgBuffer[i];
		if (crntVal < 1.189){ //corresponding to 5 cm of water depth
			corrF = 1.0;
		}
		else{
			corrF = BeamHardModel(crntVal, poly3_a, poly3_b, poly3_c, poly3_d);
		}
		pImgBuffer[i] = (float)(crntVal*corrF + poly3_e);
	}
	*/
...
void CbctRecon::SLT_DoBowtieCorrection()
{
	...
	if (ximIsUsed || hisIsUsed) {
		cout << "Bow tie filtering should not be used for His data or Xim data!!" << std::endl;
		return;
	}
	...
	{
		cout << "Wrong number of arguments!" << std::endl << "Must be a;b;c;d -> d. / (1 + exp(-b.*(x - a))) + c" << std::endl;
		return;
	}
	if (strList.length() != 5 && ui.checkBox_Fullfan->isChecked())
	{
		cout << "Wrong number of arguments!" << std::endl
			<< "Must be a;b;c;d;e -> c - sqrt(abs(a^2-((x+/-e)*d-b)^2)) * heaviside((x+/-e)*d-b+a) * heaviside(-((x+/-e)*d-b-a))" << std::endl;
		return;
	}
	...
	FloatImageType::SizeType imgSize = m_spProjImg3DFloat->GetLargestPossibleRegion().GetSize();
	if (ui.checkBox_Fullfan->isChecked())
	{
		cout << "Bow-tie correction curve:"
			<< poly3_d << " / ( 1 + exp(-" << poly3_b << " * (x - " << poly3_a << "))) + " << poly3_c
			<< " + " << poly3_d << " / ( 1 + exp(" << poly3_b << " * (x - " << poly3_e << ")))" << std::endl;
	}
	else {
		cout << "Bow-tie correction curve:" << poly3_d << " / ( 1 + exp(-" << poly3_b << " * (x - " << poly3_a << "))) + " << poly3_c << std::endl;
	}
	...
	if (ui.checkBox_Fullfan->isChecked())
	{
		while (!it.IsAtEnd())
		{
			crntVal = static_cast<double>(it.Get()); // (65535 / exp(it.Get())); //raw mu_t = ln(65535/I) <-> I = 65535 / exp(mu_t)
			x_idx = static_cast<double>(it.GetIndex()[0]) * (512.0 / imgSize[0]); // 512 from current fit -> conversion to be consistent with downResFactor
			// if (crntVal > (poly3_c - poly3_a)), negative values are fine, don't worry
			corrF = fullFan_Function(poly3_a, poly3_b, poly3_c, poly3_d, poly3_e, x_idx);
			it.Set(static_cast<float>(crntVal - corrF)); // (log(65535 / (crntVal - corrF))));
			++it;
		}
	}
	else {
	...
	SetMaxAndMinValueOfProjectionImage();
	SLT_DrawProjImages();
	cout << "Bow-tie correction done." << std::endl;
}
...
void CbctRecon::SLT_ViewRegistration() //default showing function
{
    //if (!m_spReconImg)
    /*{
      std::cout << "No fixed image is ready" << std::endl;
      return;
      }*/
    m_pDlgRegistration->UpdateListOfComboBox(0);//combo selection signalis called
    m_pDlgRegistration->UpdateListOfComboBox(1);
...
void CbctRecon::SLT_ViewHistogram() //default showing function
{
	...
	if (m_pDlgRegistration->m_spMoving) {
		ForwardProjection(m_pDlgRegistration->m_spMoving, m_spCustomGeometry, m_spProjImgCT3D, false); //final moving image
	}
...
void CbctRecon::Draw2DFrom3DDouble(UShortImageType::Pointer& spFixedImg, UShortImageType::Pointer& spMovingImg, enPLANE enPlane, double pos, YK16GrayImage& YKFixed, YK16GrayImage& YKMoving)
{
    ...
    //For moving image
    using ResampleFilterType = itk::ResampleImageFilter<UShortImageType, UShortImageType>;
    ...
    using InterpolatorType = itk::NearestNeighborInterpolateImageFunction<UShortImageType, double >;
    ...
    //cout << "Before MovingImg Creation " << std::endl;
    ...
    //cout << "tot pixel no: " << cnt << std::endl;

    //YK16GrayImage::CopyItkImage2YKImage(filter->GetOutput(), &YKMoving);

    //cout << "After MovingImg Creation " << std::endl;
...
void CbctRecon::Draw2DFrom3DDouble(UShortImageType::Pointer& spFixedImg, UShortImageType::Pointer& spMovingImg, enPLANE enPlane, double pos, AG17RGBAImage& YKFixed, AG17RGBAImage& YKMoving)
{
	if ((spFixedImg == nullptr) || (spMovingImg == nullptr)) {
		return;
}
	...
	default:
		cout << "default should not passed by" << std::endl;
		width = imgSize[0];
		height = imgSize[1];
		iCntSlice = imgSize[2];
		iReqSlice = qRound((pos - imgOrigin[2]) / imgSpacing[2]);
...
void CbctRecon::FindAllRelevantPaths(const QString& pathProjHisDir)//called following SLT_SetHisDir
{
    //in case of eletka, img_UID
    //QString aa;
    //cout<< "ddd " << aa.toLocal8Bit().constData() << std::endl;
        ...
        !curHisDir.dirName().contains("cor_", Qt::CaseSensitive))
    {
		...
		{
			cout << "XML set by guessing: Acquisitions/../Scan.xml" << std::endl;
			m_strPathGeomXML = curHisDir.absolutePath() + "/../../" + "Scan.xml";
			ui.lineEdit_ElektaGeomPath->setText(m_strPathGeomXML);
			cout << "Patient DIR set to: Acquisitions/../../../" << std::endl;
			m_strPathGeomXML = curHisDir.absolutePath() + "/../../";
			ximIsUsed = true;
			hisIsUsed = false;
			return;
			...
			cout << "Projection folder should have format [img_UID]" << std::endl;
			cout << "XML file cannot be made" << std::endl;
			return;
    ...
    else
    {
        ...
        {
            std::cout << "No found in the patient folder. DBF files can be saved in each individual patient as well. Continues to search them again in root folder(standard)" << std::endl;
        ...
        {
            std::cout << "DBF files are found in the individual patient directory." << std::endl;
        }
    ...
    //cout << "Root folder: " << tmpDir_RootFolder.absolutePath().toLocal8Bit().constData() << std::endl;
    //cout << "Root folder2: " << tmpDir_RootFolder.absolutePath().append("\\FRAME.DBF").toLocal8Bit().constData() << std::endl;
    //cout << fInfo_FrameDBF.absoluteFilePath().toLocal8Bit().constData() << std::endl;
    ...
    //break;
    // case 2:
    //cout << "No CT DICOM folder exist. Proceeding w/o CT" << std::endl;
    //break;
    // }
    //
    ...
    // {
    ////cout << listDir.at(i).absolutePath().toLocal8Bit().constData() << std::endl; //this returns Dir, not itself
    ...
    if (enDirStructure_Type != 2)
    {
        ...
        {
            std::cout << "No DCM plan file exists. Proceeding w/o dicom plan" << std::endl;
        }
        else if (listFileDCMPlan.size() > 1)
        {
            std::cout << "Warning! More than one DCM plan file exists. First DCM plan file will be used" << std::endl;
        }
        else{
    ...
    {
        std::cout << "no CBCT_RS dir exists. Proceed with out CBCT RS image" << std::endl;
    }
    else
    {
        ...
        if (listFile2.empty())
        {
            std::cout << "No CBCT DICOM RS file exist. proceeding w/o RS" << std::endl;
            //return;
            m_strPathRS_CBCT = "";
        }
        else
    ...
    std::cout << "m_strDCMUID: " << m_strDCMUID.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathPatientDir: " << m_strPathPatientDir.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPatientDirName: " << m_strPatientDirName.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathFRAME_DBF: " << m_strPathFRAME_DBF.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathIMAGE_DBF: " << m_strPathIMAGE_DBF.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathGeomXML: " << m_strPathGeomXML.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathPlanCTDir: " << m_strPathPlanCTDir.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathRS: " << m_strPathRS.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathRS_CBCT: " << m_strPathRS_CBCT.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathPlan: " << m_strPathPlan.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathElektaINI: " << m_strPathElektaINI.toLocal8Bit().constData() << std::endl;
    std::cout << "m_strPathElektaINIXVI2: " << m_strPathElektaINIXVI2.toLocal8Bit().constData() << std::endl;
    ...
    {
        //update GUI
        std::cout << "Updating current mAs setting from INI file: " << "kVp= " << kVp << ", mA= " << mA << ", ms= " << ms << std::endl;
...
#if USE_CUDA
//output spProjCT3D => intensity value, not line integral
void CbctRecon::CUDA_ForwardProjection(UShortImageType::Pointer& spVolImg3D, GeometryType::Pointer& spGeometry, UShortImageType::Pointer& spProjCT3D, bool bSave)
{
	...
	if (true)
	{
		...
		using CastFilterType = itk::CastImageFilter< UShortImageType, FloatImageType>; //Maybe not inplace filter
		CastFilterType::Pointer castFilter = CastFilterType::New();
		...
		std::cout << "CBCT calibration Factor(Recommended: 1, 0): A = " << calibF_A << "  B= " << calibF_B << std::endl;
		using MultiplyImageFilterType = itk::MultiplyImageFilter<FloatImageType, FloatImageType, FloatImageType>;
		MultiplyImageFilterType::Pointer multiplyImageFilter = MultiplyImageFilterType::New();
		multiplyImageFilter->SetInput(castFilter->GetOutput());
		multiplyImageFilter->SetConstant(calibF_A / 65535.0);

		using AddImageFilterType = itk::AddImageFilter <FloatImageType, FloatImageType, CUDAFloatImageType>;
		AddImageFilterType::Pointer addImageFilter = AddImageFilterType::New();
		...
		using ConstantImageSourceType = rtk::ConstantImageSource<CUDAFloatImageType>; //Output: FLoat image = may be mu_t = log(I_0/I)
		ConstantImageSourceType::Pointer constantImageSource = ConstantImageSourceType::New();
		...
		std::cout << "Forward projection done in:	" << projProbe.GetMean() << ' ' << projProbe.GetUnit() << '.' << std::endl;
	...
	if (bSave)
	{
		...
		cout << "Files are being saved" << std::endl;
		cout << " Patient DIR Path: " << m_strPathPatientDir.toLocal8Bit().constData() << std::endl;

		if (m_strPathPatientDir.isEmpty())
		{
			cout << "File save error!: No patient DIR name" << std::endl;
			return;
		}
		...
		if (!crntDir.exists())
		{
			cout << "File save error: The specified folder does not exist." << std::endl;
			return;
		}
		...
		if (!tmpResult)
		{
			cout << "FwdProj directory seems to exist already. Files will be overwritten." << std::endl;
		}

		QString strSavingFolder = strCrntDir + "/" + fwdDirName;
		SaveProjImageAsHIS(spProjCT3D, m_arrYKBufProj, strSavingFolder, m_fResampleF);
	}
}
#endif

//output spProjCT3D => intensity value, not line integral
void CbctRecon::CPU_ForwardProjection(UShortImageType::Pointer& spVolImg3D, GeometryType::Pointer& spGeometry, UShortImageType::Pointer& spProjCT3D, bool bSave)
{
	//m_spProjCTImg --> spProjCT3D
	...
	if (true)
	{
		...
		*/
		...
		// Create a stack of empty projection images
		using ConstantImageSourceType = rtk::ConstantImageSource<FloatImageType>; //Output: FLoat image = may be mu_t = log(I_0/I)
		ConstantImageSourceType::Pointer constantImageSource = ConstantImageSourceType::New();
		...
		// cout << "projection algorithm (0:Joseph, 1: CUDA, 2:RayCast ): " << fwdMethod << std::endl;
	...
	if (bSave)
	{
		...
		if (!tmpResult)
		{
			std::cout << "FwdProj directory seems to exist already. Files will be overwritten." << std::endl;
		}
...
void CbctRecon::SaveProjImageAsHIS(UShortImageType::Pointer& spProj3D, std::vector<YK16GrayImage> arrYKImage, QString& strSavingFolder, double resampleF)
{
    ...
    if (resampleF != 1.0)
    {
        std::cout << "restore the  resampled image by applying a factor of " << restoreResampleF << std::endl;
    ...
    for (auto it = arrYKImage.begin(); it != arrYKImage.end() && !it_FwdProj.IsAtEnd(); ++it)
    {
		...
		if (fopen_s(&fd, crntPath.toLocal8Bit().constData(), "wb") == 0) {
			std::cerr << "Could not open file: " << crntPath.toLocal8Bit().constData() << " for writing!" << std::endl;
			return;
		}
        fwrite(it->m_pElektaHisHeader, 100, 1, fd); //this buffer only include header info
        ...
        //cout << "Now saving " << i+1 << " th file: " << crntFileName.toLocal8Bit().constData() << std::endl;
    }
...
void CbctRecon::SLT_DoScatterCorrection_APRIORI()
{
    ...
    {
        std::cout << "No Moving image in Registration is found. Ref CT image will be used instead" << std::endl;
        ForwardProjection(m_spRefCTImg, m_spCustomGeometry, m_spProjImgCT3D, bExportProj_Fwd); //final moving image
    }
    else
    {
        std::cout << "Error!: No ref image for forward projection is found." << std::endl;
        return;
    }

    //YKTEMP
    std::cout << "ProjImgCT Size = " << m_spProjImgCT3D->GetBufferedRegion().GetSize()[0] << ", " << m_spProjImgCT3D->GetBufferedRegion().GetSize()[1] << ", " << m_spProjImgCT3D->GetBufferedRegion().GetSize()[2] << std::endl;
    std::cout << "ProjImgCT origin = " << m_spProjImgCT3D->GetOrigin()[0] << ", " << m_spProjImgCT3D->GetOrigin()[1] << ", " << m_spProjImgCT3D->GetOrigin()[2] << std::endl;
    std::cout << "ProjImgCT spacing = " << m_spProjImgCT3D->GetSpacing()[0] << ", " << m_spProjImgCT3D->GetSpacing()[1] << ", " << m_spProjImgCT3D->GetSpacing()[2] << std::endl;
    ...
    std::cout << "To account for the mAs values, the intensity scale factor of " << GetRawIntensityScaleFactor() << "will be multiplied during scatter correction to avoid negative scatter" << std::endl;
    ...
    std::cout << "FINISHED!Scatter correction: CBCT DICOM files are saved" << std::endl;
}
#include <itkStatisticsImageFilter.h>
void CbctRecon::CalculateIntensityScaleFactorFromMeans(UShortImageType::Pointer& spProjRaw3D, UShortImageType::Pointer& spProjCT3D) 
{
	...
	double medianRadius, double gaussianSigma, int nonNegativeScatOffset, bool bSave)
{
    ...
    if (size1[0] != size2[0] ||
        ...
        size1[2] != size2[2])
    {
        std::cout << "Raw and CT projection dimension are not matching. under the high resolution macro?" << std::endl;
		// Why 2.0 shouldn't it be 1/downresfactor ?
        if (size1[0] == static_cast<SizeType::SizeValueType>(qRound(size2[0] * 2.0))
			&& size1[1] == static_cast<SizeType::SizeValueType>(qRound(size2[1] * 2.0)) && size1[2] == size2[2])
        {
    ...
    // std::cout << "Scat3D size = " << spProjScat3D->GetRequestedRegion().GetSize() << std::endl;

    UShortImageType::SizeType imgSize = spTmpProjRaw3D->GetRequestedRegion().GetSize();
    //UShortImageType::SizeType imgSize = spProjCT3D->GetRequestedRegion().GetSize();

    //UShortImageType::SizeType imgSize = spSrcImg3D->GetBufferedRegion().GetSize();
    ...
    for (int i = 0; i < iSizeZ; i++)
    {
        ...
        using SmoothingFilterType = itk::SmoothingRecursiveGaussianImageFilter<FloatImage2DType, FloatImage2DType>;
		...
		} else
		{
			...
			gaussianFilter->SetSigmaArray(gaussianSigmaArray); //filter specific setting for 512x384 (varian/2)
		}
    ...
    if (bSave)
    {
        ...
        if (!tmpResult)
        {
            std::cout << "Scatter map directory seems to exist already. Files will be overwritten." << std::endl;
        }
...
void CbctRecon::ScatterCorr_PrioriCT(UShortImageType::Pointer& spProjRaw3D, UShortImageType::Pointer& spProjScat3D, UShortImageType::Pointer& m_spProjCorr3D, int nonNegativeScatOffset, int postMedian, bool bSave)
{
    ...
    if (size1[0] != size2[0] ||
        ...
        size1[2] != size2[2])
    {
        std::cout << "Raw and scatter projection dimension are not matching. under the high resolution macro?" << std::endl;

        if (static_cast<int>(size1[0]) == qRound(size2[0] * 2.0) && static_cast<int>(size1[1]) == qRound(size2[1] * 2.0) && size1[2] == size2[2])
        {
            bHighResolMacro = true;
        }
        else
    ...
    for (int i = 0; i < iSizeZ; i++)
    {
        ...
        if (i%unit == 0)
        {
            std::cout << "Applying scatter correction: " << (i / static_cast<double>(unit))*10.0 << " % is done" << std::endl;
        }
    }
    ...
    if (bSave)
    {
        ...
        if (!tmpResult)
        {
            std::cout << "Corrected projection directory seems to exist already. Files will be overwritten." << std::endl;
        }

        QString strSavingFolder = strCrntDir + "/" + scatDirName;

        if (!bHighResolMacro) {
            SaveProjImageAsHIS(m_spProjCorr3D, m_arrYKBufProj, strSavingFolder, m_fResampleF);
        } else {
...
void CbctRecon::Get2DFrom3D(UShortImageType::Pointer& spSrcImg3D, FloatImage2DType::Pointer& spTargetImg2D, int idx, enPLANE iDirection)
{
    ...
    //cout << "Get2DFrom3D zSize = " << zSize << std::endl;
    ...
    {
        std::cout << "something is here in target image. is it gonna be overwritten?" << std::endl;
    }
    ...
    //cout << "src size = " << spSrcImg3D->GetRequestedRegion().GetSize() << " " << std::endl;
    //cout << "target image size = " << spTargetImg2D->GetRequestedRegion().GetSize() << " " << std::endl;


    itk::ImageSliceConstIteratorWithIndex<UShortImageType> it_3D(spSrcImg3D, spSrcImg3D->GetRequestedRegion());
    //itk::ImageRegionIteratorWithIndex<FloatImageType2D> it_2D (spTargetImg2D, spTargetImg2D->GetRequestedRegion());
    ...
    for (int i = 0; i < zSize && !it_3D.IsAtEnd(); i++)
    {
        ...
        if (i == idx)
        {
            while (!it_3D.IsAtEndOfSlice()) //Error here why?
            {
                ...
                {
                    auto tmpVal = static_cast<float>(it_3D.Get()); //in proj image case, this is intensity
                    it_2D.Set(tmpVal);
                    ++it_2D;
                    ++it_3D;
                }//while2
    ...
    //cout << "cnt = " << cnt << " TotCnt " << cntTot << std::endl;
    /*YK16GrayImage tmpYK;
...
{
    if ((spSrcImg2D == nullptr) || (spTargetImg3D == nullptr)) { //Target image should be also ready.
        return;
}
    ...
    if (imgDim2D[0] != imgDim3D[idxHor] ||
        imgDim2D[1] != imgDim3D[idxVer] || idx < 0 || idx >= static_cast<int>(imgDim3D[idxZ]))
    {
    ...
    //itk::ImageRegionConstIteratorWithIndex<FloatImageType2D> it_2D (spSrcImg2D, spSrcImg2D->GetRequestedRegion());
...
//void CbctRecon::Get2DFrom3D( FloatImageType::Pointer& spSrcImg3D, FloatImageType2D::Pointer& spTargetImg2D, enPLANE iDirection)
//{
//
//}
...
void CbctRecon::ConvertIntensity2LineInt(UShortImageType::Pointer& spProjIntensity3D, FloatImageType::Pointer& spProjLineInt3D, int bkIntensity)
{
    ...
    for (it_Src.GoToBegin(), it_Tar.GoToBegin(); !it_Src.IsAtEnd() && !it_Tar.IsAtEnd(); ++it_Src, ++it_Tar)
    {
        ...
        float mu_t_val = log(background_intensity / static_cast<double>(it_Src.Get()));
        it_Tar.Set(mu_t_val);
    }
}
...
void CbctRecon::ResampleItkImage(FloatImageType::Pointer& spSrcImg, FloatImageType::Pointer& spTarImg, double resFactor)
{
    ...
    using ResampleImageFilterType = itk::ResampleImageFilter<FloatImageType, FloatImageType, float>;
	...
	using InterpolatorType = itk::NearestNeighborInterpolateImageFunction<FloatImageType, float >;
    InterpolatorType::Pointer interpolator = InterpolatorType::New();
...
void CbctRecon::ResampleItkImage(UShortImageType::Pointer& spSrcImg, UShortImageType::Pointer& spTarImg, double resFactor)
{
    ...
    //  std::cout << "original Origin: " << spSrcImg2D->GetOrigin() << std::endl;
    using ResampleImageFilterType = itk::ResampleImageFilter<UShortImageType, UShortImageType, float>;
    ResampleImageFilterType::Pointer resample = ResampleImageFilterType::New();
	...
	using InterpolatorType = itk::NearestNeighborInterpolateImageFunction<UShortImageType, float >;
    InterpolatorType::Pointer interpolator = InterpolatorType::New();
    resample->SetInterpolator(interpolator);
...
void CbctRecon::ResampleItkImage2D(FloatImage2DType::Pointer& spSrcImg2D, FloatImage2DType::Pointer& spTarImg2D, double resFactor)
{
    ...
    //  std::cout << "original Origin: " << spSrcImg2D->GetOrigin() << std::endl;
    using ResampleImageFilterType = itk::ResampleImageFilter<FloatImage2DType, FloatImage2DType, float>;
    ResampleImageFilterType::Pointer resample = ResampleImageFilterType::New();
    ...
    //cout << "Output spacing: " << outputSpacing << std::endl;

    //FloatImageType2D::Pointer input = spSrcImg2D;
    //FloatImageType2D::PointType prevOrigin = input->GetOrigin(); //-204.6 - 204.6  0
	...
	using InterpolatorType = itk::NearestNeighborInterpolateImageFunction<FloatImage2DType, float >;
    InterpolatorType::Pointer interpolator = InterpolatorType::New();
    resample->SetInterpolator(interpolator);
...
void CbctRecon::AfterScatCorrectionMacro()
{
    ...
    std::cout << "Post  FDK reconstruction is done. Moving on to post skin removal" << std::endl;
...
{
    //cout << "Before filter spImgUshort" << spImgUshort << std::endl;
    ...
    //cout << "After filter spImgUshort" << spImgUshort << std::endl;
    //cout << "After filter spImgShort" << spImgShort << std::endl;
}
...
void CbctRecon::SLT_CalcAndSaveAngularWEPL()//single point
{
    ...
    //export arrWEPL
    QString filePath = QFileDialog::getSaveFileName(this, "Save data", "", "txt image file (*.txt)", nullptr, nullptr); //Filename don't need to exist
...
void CbctRecon::SLT_DoScatterCorrectionUniform()
{
    ...
    using ScatterFilterType = rtk::BoellaardScatterCorrectionImageFilter<UShortImageType, UShortImageType>;
    ...
    std::cout << "Boallaard uniform scatter correction is being applied" << std::endl;
    std::cout << "Air threshold: " << airThre << std::endl;
    std::cout << "Scatter to Primary ratio(will be automatically adjusted by the minimum intensity): " << scat2PrimRatio << std::endl;
    std::cout << "NonNegativityConstraintThreshold: " << nonNagativity << std::endl;
    ...
    std::cout << "FINISHED!: Uniform Scatter correction for raw projection images (Boallaard method) is completed. Proceed to reconstruction" << std::endl;
}
...
{
    QString strPath = QFileDialog::getOpenFileName(this, "Open RS file", m_strPathDirDefault, "DICOM RS (*.dcm)", nullptr, nullptr);
...
void CbctRecon::SLT_CropSkinUsingThreshold()
{
	...
	using threshFilterType = itk::BinaryThresholdImageFilter <UShortImageType, UShortImageType>;
	...
	UShortImageType::SizeType imgDims = spCrntImgMask->GetBufferedRegion().GetSize();
	...
	using StructElementType = itk::BinaryBallStructuringElement< USHORT_PixelType, 3>;
	using ErodeFilterType = itk::BinaryErodeImageFilter< UShortImageType, UShortImageType, StructElementType >;
	using DilateFilterType = itk::BinaryDilateImageFilter< UShortImageType, UShortImageType, StructElementType >;
	...
	cout << "Skin mask is being created..." << std::endl;

	using MaskFilterType = itk::MaskImageFilter <UShortImageType, UShortImageType, UShortImageType>;
...
void CbctRecon::ExportAngularWEPL_byFile(QString& strPathOutput)
{
    ...
    //cout << "Computed WEPL points: " << vOutputWEPL.size() << std::endl;
    ...
    //fout << "POI_Index" << "," << "Gantry_Angle" << "," << "WEPL(mm)" << std::endl;
...
void CbctRecon::ExportAngularWEPL_byFile(QString& strPathOutput)
{
	...
	if (m_vPOI_DCM.empty())
	{
		cout << "No POI data is prepared. Load them first" << std::endl;
		return;
	}

	if (m_spRawReconImg == nullptr)
	{
		cout << "Error: no Raw Recon image is found" << std::endl;
		return;
	}

	if (m_spScatCorrReconImg == nullptr)
	{
		cout << "Warning: no ScatCorrReconImg is found" << std::endl;
	}

	if (m_spManualRigidCT == nullptr)
	{
		cout << "Warning: no ManualRigidCT is found" << std::endl;
	}
	if (m_spAutoRigidCT == nullptr)
	{
		cout << "Warning: no AutoRigidCT is found" << std::endl;
	}
...
#pragma omp section
	...
	if ((m_spScatCorrReconImg != nullptr) && static_cast<int>(vOutputWEPL_corCBCT.size()) == cntWEPL) {
		fout << "CorrCBCT" << "\t";
}
	if ((m_spManualRigidCT != nullptr) && static_cast<int>(vOutputWEPL_manual.size()) == cntWEPL) {
		fout << "ManualRigidCT" << "\t";
}
	if ((m_spAutoRigidCT != nullptr) && static_cast<int>(vOutputWEPL_auto_rigid.size()) == cntWEPL) {
		fout << "AutoRigidCT" << "\t";
}
	if ((m_spDeformedCT_Final != nullptr) && static_cast<int>(vOutputWEPL_deform.size()) == cntWEPL) {
		fout << "DeformedCT" << "\t";
}
	fout << std::endl;
...
{
    //export arrWEPL
    QString filePath = QFileDialog::getSaveFileName(this, "Save data", m_strPathDirDefault, "txt image file (*.txt)", nullptr, nullptr); //Filename don't need to exist
...
double plastimatchWEPL() {
	...
	//scene->create_patient_psp(); // hu to stopping power //not necessary when practically copied below
	//std::string flavor("b");
	...
	// ONLY way to call prepare_for_calc from the "outside" (without ugly tricks)
	...
	double WEPL = rpl_vol->get_value(ap_idx_default, rglength);
...
#if USE_AVX
double WEPL_from_point(const double cur_point[3],
	const std::array<double, 3> vec_basis,
	const double vec_cubesize[3], const size_t cubedim[3],
	const float *vec_wepl_cube) {
	const __m256d step =
		_mm256_set_pd(vec_basis[0], vec_basis[1], vec_basis[2], 0.0); // AVX

	const __m256d cubedim_rev = _mm256_set_pd(
		static_cast<double>(cubedim[0]), static_cast<double>(cubedim[1]),
		static_cast<double>(cubedim[2]), 1.0); // 2->0 for convienience
	const __m256d inv_cubesize = _mm256_set_pd(
		1.0 / vec_cubesize[0], 1.0 / vec_cubesize[1], 1.0 / vec_cubesize[2], 0.0);
	const __m256d zeros = _mm256_set1_pd(0.0);

	const __m128i point_id_to_cube_ids = _mm_set_epi32( // SSE2
		static_cast<int>(cubedim[2] * cubedim[1]), static_cast<int>(cubedim[2]),
		1, 0);
	...
	while (true)
	{
		...
		// Break if outside cube // 4th element is always False, so if an AVX "OR" instruction returning int exists, it should be used here
#ifdef _WIN32
		if ((any_true.m256d_f64[0] != 0.0) ||
			(any_true.m256d_f64[1] != 0.0) ||
			(any_true.m256d_f64[2] != 0.0)) {
			break;
		}
#else
		if ((any_true[0] != 0.0) ||
...
#ifdef _WIN32
		out += vec_wepl_cube[static_cast<size_t>(cube_id.m128i_i32[0] +
			cube_id.m128i_i32[1] +
			cube_id.m128i_i32[2])];
#else
	...
	const FloatImageType::Pointer& vec_wepl_cube)
{
	...
	const std::array<double, 3> step = { { 
			vec_basis.at(0) * step_length, 
			vec_basis.at(1) * step_length, 
			vec_basis.at(2) * step_length
		} };

	const std::array<double, 3> inv_cubesize = { {
		1.0 / vec_cubesize.at(0), 1.0 / vec_cubesize.at(1), 1.0 / vec_cubesize.at(2) } };
	...
	while (true)
	{
		...
		if (point_id.at(0) < 0.0 || point_id.at(0) >= static_cast<int>(cubedim.at(0)) ||
			point_id.at(1) < 0.0 || point_id.at(1) >= static_cast<int>(cubedim.at(1)) ||
			point_id.at(2) < 0.0 || point_id.at(2) >= static_cast<int>(cubedim.at(2))) {
			break;
		}

		// get nearest neighbors:
		const std::array<double, 3> point_id_pos = { {
				point.at(0) * inv_cubesize.at(0),
				point.at(1) * inv_cubesize.at(1),
		...
		if ((point_id.at(0) + idx_2) < 0.0 || (point_id.at(0) + idx_2) >= static_cast<int>(cubedim.at(0)) ||
			(point_id.at(1) + idy_2) < 0.0 || (point_id.at(1) + idy_2) >= static_cast<int>(cubedim.at(1)) ||
			(point_id.at(2) + idz_2) < 0.0 || (point_id.at(2) + idz_2) >= static_cast<int>(cubedim.at(2))) {
			break;
}
...
void CbctRecon::GetAngularWEPL_window(UShortImageType::Pointer& spUshortImage, float fAngleGap, float fAngleStart, float fAngleEnd, std::vector<WEPLData>& vOutputWEPLData, bool bAppend)
{
	...
	const std::array<double, 3> pixel_size = { {
		spUshortImage->GetSpacing()[0],
...
void CbctRecon::GetAngularWEPL_SinglePoint(UShortImageType::Pointer& spUshortImage, float fAngleGap, float fAngleStart, float fAngleEnd, VEC3D calcPt, int curPtIdx, vector<WEPLData>& vOutputWEPLData, bool bAppend)
{
	...
	const std::array<size_t, 3> isoTarget = { {
			static_cast<size_t>(calc_idx[0]),
			static_cast<size_t>(calc_idx[1]),
			static_cast<size_t>(calc_idx[2])
	} };

	std::cout << "Target: ( " << isoTarget[0] << ", " << isoTarget[1] << ", " << isoTarget[2] << " ), sizeAngles: " << sizeAngles << std::endl;
	...
	const std::array<double, 3> pixel_size = { {
			wepl_image->GetSpacing()[0], wepl_image->GetSpacing()[1], wepl_image->GetSpacing()[2]
		} };
    ...
    for (auto it : stArrWEPL)
    {
		...
		std::printf("%.2f, [%.2f, %.2f, %.2f]: ", curAngle, basis[0], basis[1], basis[2]);
	...
	vOutputWEPLData.insert(vOutputWEPLData.end(), stArrWEPL.begin(), stArrWEPL.end());
	/*
    for (int i = 0; i < sizeAngles; i++)
    {
	...
	if (err == 0) {
		std::cerr << "Ran into an error: " << err << " upon changing stdout stream" << std::endl;
}
	
	err = freopen_s(&stream_err, "CON", "w", stderr);
	if (err == 0) {
		std::cerr << "Ran into an error: " << err << " upon changing stderr stream" << std::endl;
}
}
...
#pragma omp parallel for
	for (int p = 0; p < static_cast<int>(sizePOI); p++)
	{
		...
		for (int i = 0; i < sizeAngles; i++)
		{
			...
			curWEPLData.fWEPL = static_cast<float>(rpl_vol->get_value(ap_idx_default, rglength)); //rgdepth
...
void CbctRecon::SLT_LoadPOIData()//it fills m_vPOI_DCM
{
    ...
    QString filePath = QFileDialog::getOpenFileName(this, "POI data file", m_strPathDirDefault, "POI data file (*.txt)", nullptr, nullptr);
    ...
    {
        std::cout << "Data " << i << "	" << m_vPOI_DCM.at(i).x << ", " << m_vPOI_DCM.at(i).y << ", " << m_vPOI_DCM.at(i).z << std::endl;
    }
    std::cout << "POI data has been loaded. " << m_vPOI_DCM.size() << " data points are read" << std::endl;
    fin.close();
}
...
void CbctRecon::SLT_StopSyncFromSharedMem()
{
    ...
    while (cnt < max_cnt)
    {
        ...
        case WAIT_OBJECT_0: //0 -->This got the semaphore token
            ...
            fout << "Thread %lu: wait succeeded " << GetCurrentThreadId() << std::endl;
            Sleep(10);
        ...
        case WAIT_TIMEOUT: //no need of release //258
            ...
            fout << "Thread %lu: wait timed out " << GetCurrentThreadId() << std::endl;
            break;
        }
        //	ReleaseSemaphore(hSemaphore, 1, &prev_counter);
    }
...
void CbctRecon::SLT_TimerEvent()
{
    ...
    charBuf = static_cast<unsigned char*>(MapViewOfFile(handle, FILE_MAP_READ, 0, 0, size));

    if (charBuf == nullptr)
    {
        std::cout << "Shared memory was not read. Timer will be stopped" << std::endl;
        SLT_StopSyncFromSharedMem();
        CloseHandle(handle);
        delete[] charBuf;
        return;
    ...
    for (int i = 0; i < pix_size; i++)
    {
        ...
        idxB = i * 2;
        //0: 1,0  1: 3,2 ...
        m_arrYKImage.at(0).m_pData[i] = ((charBuf[idxA] << 8) | charBuf[idxB]); //little endian
    }
...
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "rtk output float image (*.mha)", nullptr, nullptr);
...
void CbctRecon::MedianFilterByGUI()
{
    ...
    if (indexRadius[0] != 0 || indexRadius[1] != 0 || indexRadius[2] != 0)
    {
        ...
        //this is radius. 1 --> median window 3
        std::cout << "Post median(3D) filtering is under progress..Size(radius X Y Z) is = " << indexRadius << std::endl;
...
void CbctRecon::SLT_Export2DDose_TIF() //2D dose from current displayed image of reconstruction
{
    ...
    QString strPath = QFileDialog::getSaveFileName(this, "Save Image", "", "signed short meta image (*.tif)", nullptr, nullptr);
    if (strPath.length() <= 1) {
        return;
}

    auto originLeft = static_cast<double>(m_spCrntReconImg->GetOrigin()[0]);
    auto originTop = static_cast<double>(m_spCrntReconImg->GetOrigin()[1]);//not sure...

    auto spacingX = static_cast<double>(m_spCrntReconImg->GetSpacing()[0]);
    auto spacingY = static_cast<double>(m_spCrntReconImg->GetSpacing()[1]);//not sure...
...
void CbctRecon::SLTM_Export2DDoseMapAsMHA()
{
    ...
    QString strPath = QFileDialog::getSaveFileName(this, "Save Image", "", "itk compatible meta image (*.mha)", nullptr, nullptr);
    if (strPath.length() <= 1) {
        return;
}
    ...
    {
        pixel_val = static_cast<double>(m_dspYKReconImage->m_pData[i]) * factor_ushort2float;
        it.Set(pixel_val);
        i++;
    }
    //YK201502
...
void CbctRecon::SLTM_ExportProjGeometryTXT()
{
    ...
    if (m_spCustomGeometry == nullptr) { //will be filled after Projection load button is pushed
        return;
}

    QString strPath = QFileDialog::getSaveFileName(this, "Save text file", "", "text (*.txt)", nullptr, nullptr);
    ...
    if (cntAngle <= 0)
    {
        std::cout << "Error! no angle vector is found" << std::endl;
        return;
    }
    ...
    {
        std::cout << "Error! Angle number and shift number are not matching." << std::endl;
        return;
    }
    ...
    fout << "MV_Gantry_Angle" << "	" << "PanelShiftX(mm)" << "	" << "PanelShiftY(mm)" << std::endl;
...
void CbctRecon::SLTM_ForwardProjection()
{
    ...
    {
        std::cout << "No geometry is ready. moving on to 360 projection" << std::endl;
...
void CbctRecon::SLTM_BatchScatterCorrectionMacroAP()
{
    ...
    for (int i = 2; i < iCntProjDir; i++)// start from 2
    {
        ...
        {
            std::cout << "Found projection dir number: " << cntHisDir << ", Current Proj Path: " << curProjDirPath.toLocal8Bit().constData() << std::endl;
    ...
    std::cout << "Batch mode calculation is done! " << QString::number(elapsedSec, 'f', 2).toLocal8Bit().constData() << " seconds was spent for " << cntHisDir << " cases" << std::endl;
}
...
bool CbctRecon::FullScatterCorrectionMacroSingle(QString& outputDirPath, enREGI_IMAGES enFwdRefImg, bool bFullResolRecon, bool bExportImages, bool bCBCT_IntensityShift)
{
    ...
    if (!m_bMacroContinue)
    {
        std::cout << "Stopped during MacroSingle due to error in PreProcessCT" << std::endl;
        return false;
    }
    ...
    while (!it.IsAtEnd())
    {
        ...
        while (!it.IsAtEndOfSlice())
        {
            ...
            while (!it.IsAtEndOfLine())
            {
                ...
                crntPhysX = iPosX*static_cast<double>(spacing[0]) + static_cast<double>(origin[0]);
                crntPhysY = iPosY*static_cast<double>(spacing[1]) + static_cast<double>(origin[1]);
                crntPhysZ = iNumSlice*static_cast<double>(spacing[2]) + static_cast<double>(origin[2]);
    ...
    while (!it.IsAtEnd())
    {
        ...
        while (!it.IsAtEndOfSlice())
        {
            ...
            while (!it.IsAtEndOfLine())
            {
                ...
                if (pow(crntPhysX, 2.0) + pow(crntPhysY, 2.0) + pow(crntPhysZ, 2.0) < pow(sphereR, 2.0))
                {
                    devSum = devSum + pow((static_cast<double>(it.Get()) - meanIntensity), 2.0);
                }
                ++it;
                iPosX++;
            }
...
void CbctRecon::SLT_OpenPhaseData()
{
    ...
    //Open file
    QString filePath = QFileDialog::getOpenFileName(this, "Open phase text", m_strPathDirDefault, "Phase text file (*.txt)", nullptr, nullptr);
    ...
    fin.close();
    std::cout << "NumOfPhaseData[Float]= " << phaseCnt << "  Mean Phase value= " << phaseSum / static_cast<double>(phaseCnt) << std::endl;
}
...
void CbctRecon::SLT_Export4DCBCT()
{
    ...
    {
        std::cout << "Size not matched. NumOfProjection= " << NumOfGanAngle << " NumOfProjection= " << NumOfPhase << std::endl;
        return;
    }
    //build phase bins
    ...
    for (int i = 0; i < cntGroup; i++)
    {
        ...
        {
            std::cout << "Error in ResortCBCTProjection " << strlistPhaseFull.at(i).toLocal8Bit().constData() << std::endl;
            return;
        }
    }
...
bool CbctRecon::ResortCBCTProjection(vector<int>& vIntPhaseBinSelected, QString& strPathForXML, QString& strPathProjRoot, QString& strUID, vector<float>& vFloatPhaseFull, GeometryType::Pointer& spGeomFull, vector<string>& vProjPathsFull)
{
    ...
    {
        std::cout << "Num of data is not matching:" << " NumOfPhaseFull= " << NumOfPhaseFull << " NumOfGeomFull= " << NumOfGeomFull << " NumOfProjFileFull= " << NumOfProjFileFull << std::endl;
        return false;
    }
...
{
    QString fileName = QFileDialog::getOpenFileName(this, "Open Image", m_strPathDirDefault, "Short image file (*.mha)", nullptr, nullptr);
    ...
    //cout << m_spScatCorrReconImg->GetBufferedRegion().GetSize() << std::endl;
...
void CbctRecon::SLT_DoCouchCorrection()
{
    ...
    std::cout << "Couch shift and rotation was successfully applied." << std::endl;
}
...
void CbctRecon::SLTM_WELPCalcMultipleFiles()
{
    ...
    QString strPathOutText = QFileDialog::getSaveFileName(this, "File path to save", m_strPathDirDefault, "WEPL_value (*.txt)", nullptr, nullptr); //Filename don't need to exist
    if (strPathOutText.length() <= 1) {
        return;
}


	std::vector<vector<WEPLData>> vArrOutputWEPL;
	vArrOutputWEPL.resize(iCntFiles);

    for (int i = 0; i < iCntFiles; i++)
    {
        GetWEPLDataFromSingleFile(listFilePath.at(i), m_vPOI_DCM, vArrOutputWEPL.at(i));
    }
    ...
    for (int i = 0; i < cntWEPL; i++)
    {
        fout << vArrOutputWEPL.at(0).at(i).ptIndex << "\t" << vArrOutputWEPL.at(0).at(i).fGanAngle << "\t" << i;
...
void CbctRecon::GetWEPLDataFromSingleFile(const QString& filePath, vector<VEC3D>& vPOI, vector<WEPLData>& vOutputWEPL)
{
    ...
    for (int i = 0; i < iCntPOI; i++)
    {
        ...
        //append mode
        GetAngularWEPL_SinglePoint(spImg, fAngleGap, fAngleStart, fAngleEnd, curPOI, i, vOutputWEPL, true);//mandatory
    }
}
...
void CbctRecon::SLTM_ScatterCorPerProjRef() //load text file
{
    ...
    {
        std::cout << "Error! Ref Vol list is not ready yet. Load it first" << std::endl;
        return;
    }
    ...
    //else
    //{
    //    std::cout << "Mask file of auto-registration is not prepared. Use manual regi-mask instead" << std::endl;
    ...
    //    else
    //    {
    //        std::cout << "Mask file of manual registration is not prepared. Skip skin removal!" << std::endl;
    //        return;
    //    }
    //}
    ...
    //{
    //    std::cout << "NO plastimatch Dir was defined. CorrCBCT will not be saved automatically" << std::endl;
    //    return;
    //}
    //Forward proj
    ...
    for (int i = 0; i < iCntRefVol; i++)
    {
        ...
        {
            std::cout << "Error! in " << i << " th image. File couldn't be found. Path= " << strDirPath.toLocal8Bit().constData() << std::endl;
            return;
        }
    ...
    //cout << "Writing the CBCT file" << std::endl;
    //writer->Update();

    //QFileInfo CBCTInfo(filePathCBCT);
    //if (!CBCTInfo.exists())
    //{
    //    std::cout << "No CBCT file to read. Maybe prior writing failed" << std::endl;
    //    return;
    //}

    ////ERROR HERE! delete the temporry folder.
    //cout << "Delete the temporary folder if it crashes" << std::endl;
    ...
    UShortImageType::Pointer& spProjImg3D, int iSliceIdx)
{
    ...
    //cout << "Setting-up vacant projection image data" << std::endl;
    ...
    {
        std::cout << "Error! totalProjSize= " << totalProjSize << " iSliceIdx= " << iSliceIdx << std::endl;
    }
    ...
    //cout << "Canvas for projection image is ready to write" << std::endl;

    //4) Prepare CT image to be projected
    int fwdMethod = en_CudaRayCast; //later, it will be coming from the GUI
    //    std::cout << "projection algorithm (0:Joseph, 1: CUDA, 2:RayCast ): " << fwdMethod << std::endl;
    ...
    itk::TimeProbe projProbe;
    //cout << "Forward projection is now ongoing" << std::endl;
    ...
    std::cout << "Forward projection done by in method ID = " << fwdMethod << " in:	" << projProbe.GetMean() << ' ' << projProbe.GetUnit() << '.' << std::endl;
    ...
    //cout << "MaxValProj=" << maxValProj << " MInval=" << minValProj << " ValOffset = " << valOffset << std::endl;
...
{
    QString filePath = QFileDialog::getOpenFileName(this, "PerProjVol list", m_strPathDirDefault, "File path list (*.txt)", nullptr, nullptr);
    ...
    std::cout << m_strListPerProjRefVol.count() << " image paths were found" << std::endl;
...
//
//void CbctRecon::cudaMedianFilter2DITK(FloatImageType2D::Pointer& spFloatImage2D, int wndSizeX, int wndSizeY)
//{
//	cout << "cudaMedianFilter2DITK is being used" << std::endl;
//
//	FloatImageType2D::SizeType sizeSrc = spFloatImage2D->GetBufferedRegion().GetSize();
//	FloatImageType2D::IndexType startSrc = spFloatImage2D->GetBufferedRegion().GetIndex();
//	FloatImageType2D::SpacingType spacingSrc = spFloatImage2D->GetSpacing();
//	FloatImageType2D::PointType originSrc = spFloatImage2D->GetOrigin();
//
...
//
//	itk::ImageRegionConstIterator<FloatImageType2D> itSrc(spFloatImage2D, spFloatImage2D->GetRequestedRegion()); //writing
//
//	//itSrc.GoToBegin();
//
//	int arrIdx = 0;
...
//	itk::ImageRegionIterator<FloatImageType2D> itTarg(spFloatImage2D, spFloatImage2D->GetRequestedRegion()); //writing
//
//	itTarg.GoToBegin();
//
//	arrIdx = 0;
...
//double CbctRecon::CropSkinUsingRS(UShortImageType::Pointer& spImgUshort, QString& strPathRS, double cropMargin )
//{
...
bool SaveDoseGrayImage(const char* filePath, int width, int height, double spacingX, double spacingY, double originLeft_mm, double originTop_mm, unsigned short* pData) //export dose array to a specified file (16bit TIF)
{
    ...
    m_rXResol.a = static_cast<long>(qRound(1 / spacingX *25.4*m_rXResol.b)); //dpi
	...
	*/
    if (m_iSubFileType >= 0)
    {
		...
		//InsertHeaderToArray(&IFDArr[0], idx, TagID, dataType, DataCnt, dataVal);//dataVal이 초기값이면 insert 안함
		//idx++;
    }
...
void CbctRecon::ConvertUshort2AttFloat(UShortImageType::Pointer& spImgUshort, FloatImageType::Pointer& spAttImgFloat)
{
	...
	using AddImageFilterType = itk::AddImageFilter <FloatImageType, FloatImageType, FloatImageType>;
    AddImageFilterType::Pointer addImageFilter = AddImageFilterType::New();
    addImageFilter->SetInput1(multiplyImageFilter->GetOutput());
    double addingVal = calibF_B / 65535.0;
    addImageFilter->SetConstant2(addingVal);
...
FloatImageType::Pointer CbctRecon::ConvertUshort2WeplFloat(UShortImageType::Pointer& spImgUshort)
{
	...
	const double_pair_list lookup = { { // Data from TRiP: 19990218.hlut
	...
	std::function<float(float)> hu_to_dEdx = [&lookup](float val)
	{
		...
		auto lookup_upper_ptr = std::find_if(lookup.begin(), lookup.end(),
			[val](std::pair<float, double> cur_pair) { return val < cur_pair.first; });
		...
		const double a = (lookup_upper.second - lookup_lower.second) / static_cast<double>(lookup_upper.first - lookup_lower.first);
		const double b = lookup_upper.second - a * static_cast<double>(lookup_upper.first);
	...
	itk::ImageRegionIterator<FloatImageType> it(wepl_image, wepl_image->GetLargestPossibleRegion());
	
	// std::for_each(it.Begin(), it.End(), hu_to_dEdx); //ITK iterators doesn't support <algorithm> (yet?)
	for (it.GoToBegin(); !it.IsAtEnd(); ++it) {
...
void CbctRecon::SLTM_CropMaskBatch()
{
    ...
    QString maskFilePath = QFileDialog::getOpenFileName(this, "Mask image (Ushort)", m_strPathDirDefault, "3D mask file (*.mha)", nullptr, nullptr);
...
bool CbctRecon::SaveCurrentSetting(QString& strPathConfigFile)
{
    ...
    {
        std::cout << "Config file is found. it will be overwritten now" << std::endl;
    }
    ...
    fout << "strRefmAs" << "\t" << strRefmAs.toLocal8Bit().constData() << std::endl;
    fout << "PostFOV_R" << "\t" << PostFOV_R.toLocal8Bit().constData() << std::endl;
    fout << "PostTablePosY" << "\t" << PostTablePosY.toLocal8Bit().constData() << std::endl;

    fout << "strBkFillCT" << "\t" << strBkFillCT.toLocal8Bit().constData() << std::endl;
    fout << "strBkDetectCBCT" << "\t" << strBkDetectCBCT.toLocal8Bit().constData() << std::endl;
    fout << "strBubFillCBCT" << "\t" << strBubFillCBCT.toLocal8Bit().constData() << std::endl;

    fout << "strBkFillCBCT" << "\t" << strBkFillCBCT.toLocal8Bit().constData() << std::endl;
    fout << "strBkDetectCBCT" << "\t" << strBkDetectCBCT.toLocal8Bit().constData() << std::endl;
    fout << "strBubFillCBCT" << "\t" << strBubFillCBCT.toLocal8Bit().constData() << std::endl;

    fout << "strCropContourName" << "\t" << strCropContourName.toLocal8Bit().constData() << std::endl;

    fout << "strFOVPos" << "\t" << strFOVPos.toLocal8Bit().constData() << std::endl;
    fout << "strArgument1" << "\t" << strArgument1.toLocal8Bit().constData() << std::endl;
    fout << "strArgument2" << "\t" << strArgument2.toLocal8Bit().constData() << std::endl;
    fout << "strArgument3" << "\t" << strArgument3.toLocal8Bit().constData() << std::endl;

    fout << "bExportFwd" << "\t" << bExportFwd << std::endl;
    fout << "bExportScat" << "\t" << bExportScat << std::endl;
    fout << "bExportCor" << "\t" << bExportCor << std::endl;
    fout << "bExportVolDICOM" << "\t" << bExportVolDICOM << std::endl;
    fout << "bCouchShiftAddToMacro" << "\t" << bCouchShiftAddToMacro << std::endl;

    fout << "bCropBkgroundCT" << "\t" << bCropBkgroundCT << std::endl;
    fout << "bCropBkgroundCBCT" << "\t" << bCropBkgroundCBCT << std::endl;

    fout << "bFillBubbleCT" << "\t" << bFillBubbleCT << std::endl;
    fout << "bFillBubbleCBCT" << "\t" << bFillBubbleCBCT << std::endl;

    fout << "bUseROIForRigid" << "\t" << bUseROIForRigid << std::endl;
    fout << "bUseROIForDIR" << "\t" << bUseROIForDIR << std::endl;

    fout << "radioButton_mse" << "\t" << bRadioButton_mse << std::endl;
    fout << "radioButton_mi" << "\t" << bRadioButton_mi << std::endl;
