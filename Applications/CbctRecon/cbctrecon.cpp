...
void CbctRecon::LoadRawHndImages()
{
	...
	typedef rtk::HndImageIO readerType;
...
void CbctRecon::CudaDoReconstructionFDK(enREGI_IMAGES target)
{
	...
	typedef itk::ResampleImageFilter <CUDAFloatImageType, CUDAFloatImageType> ResampleFilterType;
	...
	typedef itk::MultiplyImageFilter<FloatImageType, FloatImageType, CUDAFloatImageType> MultiplyImageFilterType;
	MultiplyImageFilterType::Pointer multiplyImageFilter = MultiplyImageFilterType::New();
	...
	multiplyImageFilter->SetConstant(65536); //calculated already
											 ...
											 //typedef unsigned short FinalPixelType;
											 //typedef itk::Image< FinalPixelType, 3 > FinalImageType;
	typedef itk::CastImageFilter< CUDAFloatImageType, UShortImageType > CastFilterType;
	CastFilterType::Pointer castFilter = CastFilterType::New();
	...
	typedef itk::CastImageFilter <CUDAFloatImageType, FloatImageType > castBackType;
...
#if USE_OPENCL_PLM
FloatImageType::Pointer PlastimatchOpenCLFDK(FloatImageType::Pointer spCurImg, 
	rtk::ThreeDCircularProjectionGeometry::Pointer m_spCustomGeometry,
	float spacing[3], plm_long sizeOutput[3])
{
	spCurImg->Update();
	...
	offset[0] = -0.5*sizeOutput[0] * spacing[0];
	offset[1] = -0.5*sizeOutput[1] * spacing[1];
	offset[2] = -0.5*sizeOutput[2] * spacing[2];

	Volume* vol = new Volume(sizeOutput, offset, spacing, 0, PT_FLOAT, 1);
	...
	opencl_idx(ocl_proj_dim, 0) = proj_dim[0];
	opencl_idx(ocl_proj_dim, 1) = proj_dim[1];
	...
	/* Calculate the scale */
	const float scale = (float)(sqrt(3.0) / (double)(1 + proj_dim[2])) * 10.0f;
	...
	for (int image_num = 0; image_num < proj_dim[2]; image_num++)
	{
		/* Translate image and properties to PLM */
		Proj_image* proj = new Proj_image;
		proj->init();
		...
		proj->pmat->set(cam, tgt, vup, sid, proj->pmat->ic, ps); // , proj->dim);
		...
		float ic[] = { (float)proj->pmat->ic[0], (float)proj->pmat->ic[1] };
		opencl_buf_write(&ocl_dev, ocl_buf_ic, 2 * sizeof(float), ic);

		/* Copy nrm to device (convert from double to float) */
		float nrm[] = {
			(float)proj->pmat->nrm[0],
			(float)proj->pmat->nrm[1],
			(float)proj->pmat->nrm[2]
		};
...
#if USE_OPENCL_RTK
FloatImageType::Pointer RTKOpenCLFDK(FloatImageType::Pointer spCurImg,
	rtk::ThreeDCircularProjectionGeometry::Pointer m_spCustomGeometry,
	FloatImageType::SpacingType spacing,
	FloatImageType::SizeType sizeOutput,
	const double fdk_options[5])
{
	spCurImg->Update();
	// Generate image sources for cone beam CT reconstruction
	...
	const double fTruncCorFactor = fdk_options[0];
	const double fHannCut = fdk_options[1];
	const double fCosineCut = fdk_options[2];
	const double fHamming = fdk_options[3];
	const double fHannCutY = fdk_options[4];
...
QString getBowtiePath(QWidget* parent, QDir calDir) {
	return QFileDialog::getOpenFileName(parent,
		"Find air(+bowtie) filter image for subtraction", 
		calDir.absolutePath(), "Projection (*.xim)", 0, 0);
}

std::tuple<bool, bool> CbctRecon::probeUser(QString guessDir) {
...
void CbctRecon::SLT_LoadSelectedProjFiles()//main loading fuction for projection images
{
    ...
    if (ui.Radio_ManualProjAngleGap->isChecked())
    {
        ...
        for (int i = 0; i < vNormAngles.size(); i++)
        {
            ...
            std::cout << vNormAngles.at(i) << std::endl;
        }
...
// #pragma omp parallel for private(itIdx) shared(m_spCustomGeometry) schedule(static)
	...
	typedef rtk::ProjectionsReader< FloatImage2DType > FilterReaderType; //This one does a bit more than we need, but mainly statistical filters
	...
	if (correctionValue != 10.0 && correctionValue != 20.0) { // 10 and 20 are error codes
		...
		originalMax = -1.0;
		originalMin = -1.0;
		if (GetMaxAndMinValueOfProjectionImage(originalMax, originalMin, reader->GetOutput(), theoreticalMin) != 20.0)
			cout << "Reader Max, Min=" << originalMax << "	" << originalMin << std::endl;
	}
...
void CbctRecon::GetExcludeIndexByNames(QString outlierListPath, vector<string>& vProjFileFullPath, vector<int>& vExcludeIdx)
{
    ...
    while (!fin.eof())
    {
        ...
        std::cout << "Outlier file: " << str << std::endl;
...
double CbctRecon::GetMaxAndMinValueOfProjectionImage(double& fProjImgValueMax, double& fProjImgValueMin, FloatImageType::Pointer projImage, const double theoreticalMin)
{
    /*if (m_iImgCnt > 0)
    {
    m_fProjImgValueMax = 65535;
...
inline const double HndBeamHardModel(const double val)
{   //           a *       x^3     +       b *       x^2     +       c *   x +       d
	return 6.0e-08 * pow(val, 3.0) - 1.0e-08 * pow(val, 2.0) - 5.0e-07 * val + 8.0e-01;
}

inline const double HisBeamHardModel(const double val)
{   //             a *       x^3     +         b *       x^2     +         c *   x +         d
	return 9.321e-05 * pow(val, 3.0) - 2.609e-03 * pow(val, 2.0) + 3.374e-02 * val + 9.691e-01;
}

inline const double XimBeamHardModel(const double val)
{   //           a *       x^3     +      b *       x^2     +      c *   x +   d
...
void CbctRecon::SLT_DoBowtieCorrection()
{
	...
	if (ximIsUsed || hisIsUsed) {
		cout << "Bow tie filtering should not be used for His data or Xim data (from Scripps) !!" << std::endl;
		return;
	}
...
void CbctRecon::FindAllRelevantPaths(QString pathProjHisDir)//called following SLT_SetHisDir
{
        ...
        !curHisDir.dirName().contains("cor_", Qt::CaseSensitive))
    {
		...
		else if (curHisDir.absolutePath().contains("Acquisitions", Qt::CaseSensitive))
		{
...
{
	typedef itk::StatisticsImageFilter<UShortImageType> StatFilterType;
	double rawMean = 0.0;
	double ctMean = 0.0;
	...
	double medianRadius, double gaussianSigma, int nonNegativeScatOffset, bool bSave)
{
    ...
    for (int i = 0; i < iSizeZ; i++)
    {
        ...
        if (i%unit == 0)
        {
            std::cout << "Generating scatter map: " << (i / (double)unit)*10.0 << " % is done" << std::endl;
        }
    }//end of for
...
void CbctRecon::ScatterCorr_PrioriCT(UShortImageType::Pointer& spProjRaw3D, UShortImageType::Pointer& spProjScat3D, UShortImageType::Pointer& m_spProjCorr3D, int nonNegativeScatOffset, int postMedian, bool bSave)
{
    ...
    for (int i = 0; i < iSizeZ; i++)
    {
        ...
        if (i%unit == 0)
        {
            std::cout << "Applying scatter correction: " << (i / (double)unit)*10.0 << " % is done" << std::endl;
        }
    }
...
void CbctRecon::ConvertIntensity2LineInt(UShortImageType::Pointer& spProjIntensity3D, FloatImageType::Pointer& spProjLineInt3D, int bkIntensity)
{
	...
	const double background_intensity = static_cast<double>(bkIntensity);

    for (it_Src.GoToBegin(), it_Tar.GoToBegin(); !it_Src.IsAtEnd() && !it_Tar.IsAtEnd(); ++it_Src, ++it_Tar)
    {
        //mu = ln(I_0/I) OR mu = ln(I/I0)
        float mu_t_val = log(background_intensity / (double)it_Src.Get());
        it_Tar.Set(mu_t_val);
    }
}
...
void CbctRecon::SLT_CalcAndSaveAngularWEPL()//single point
{
    ...
    {
        fout << vOutputWEPL.at(i).fGanAngle << "	" << vOutputWEPL.at(i).fWEPL << std::endl;
    }
...
void CbctRecon::SLT_CropSkinUsingThreshold()
{
	...
	typedef itk::BinaryThresholdImageFilter <UShortImageType, UShortImageType> threshFilterType;
	...
	typedef itk::BinaryFillholeImageFilter <UShortImageType> HoleFillingFilterType;
	...
	typedef itk::BinaryErodeImageFilter< UShortImageType, UShortImageType, StructElementType > ErodeFilterType;
	typedef itk::BinaryDilateImageFilter< UShortImageType, UShortImageType, StructElementType > DilateFilterType;
	...
	typedef itk::MaskImageFilter <UShortImageType, UShortImageType, UShortImageType> MaskFilterType;
	...
	const FloatImageType::Pointer& vec_wepl_cube)
{
	...
	const std::array<double, 3> step = { { 
			vec_basis[0] * step_length, 
			vec_basis[1] * step_length, 
			vec_basis[2] * step_length
		} };

	const std::array<double, 3> inv_cubesize = { {
		1.0 / vec_cubesize[0], 1.0 / vec_cubesize[1], 1.0 / vec_cubesize[2] } };

	std::array<double, 3> point = { {
		static_cast<double>(cur_point_id[0]) * vec_cubesize[0],
		static_cast<double>(cur_point_id[1]) * vec_cubesize[1],
		static_cast<double>(cur_point_id[2]) * vec_cubesize[2]
		} };
	...
	while (true)
	{
		...
		const std::array<int, 3> point_id = { { 
				static_cast<int>(round(point[0] * inv_cubesize[0])),
				static_cast<int>(round(point[1] * inv_cubesize[1])),
				static_cast<int>(round(point[2] * inv_cubesize[2]))
			} };

		if (point_id[0] < 0.0 || point_id[0] >= cubedim[0] ||
			point_id[1] < 0.0 || point_id[1] >= cubedim[1] ||
			point_id[2] < 0.0 || point_id[2] >= cubedim[2]) {
			break;
		}

		// get nearest neighbors:
		const std::array<double, 3> point_id_pos = { {
				point[0] * inv_cubesize[0],
				point[1] * inv_cubesize[1],
				point[2] * inv_cubesize[2]
			} };

		int idx_2 = -1;
		if (point_id[0] < (point_id_pos[0]))
			idx_2 = 1;
		
		int idy_2 = -1;
		if (point_id[1] < (point_id_pos[1]))
			idx_2 = 1;

		int idz_2 = -1;
		if (point_id[2] < (point_id_pos[2]))
			idz_2 = 1;

		if ((point_id[0] + idx_2) < 0.0 || (point_id[0] + idx_2) >= cubedim[0] ||
			(point_id[1] + idy_2) < 0.0 || (point_id[1] + idy_2) >= cubedim[1] ||
			(point_id[2] + idz_2) < 0.0 || (point_id[2] + idz_2) >= cubedim[2])
			break;

		double weights[8];
		// x                 xyz
		weights[0] = sqrt(// 000 =
			pow(point_id[0] - point_id_pos[0], 2) +
			pow(point_id[1] - point_id_pos[1], 2) +
			pow(point_id[2] - point_id_pos[2], 2));
		weights[1] = sqrt(// 100 =
			pow(point_id[0] + idx_2 - point_id_pos[0], 2) +
			pow(point_id[1] - point_id_pos[1], 2) +
			pow(point_id[2] - point_id_pos[2], 2));
		// y
		weights[2] = sqrt(// 010 =
			pow(point_id[0] - point_id_pos[0], 2) +
			pow(point_id[1] + idy_2 - point_id_pos[1], 2) +
			pow(point_id[2] - point_id_pos[2], 2));
		weights[3] = sqrt(// 110 =
			pow(point_id[0] + idx_2 - point_id_pos[0], 2) +
			pow(point_id[1] + idy_2 - point_id_pos[1], 2) +
		...
		weights[4] = sqrt(// 001 =
			pow(point_id[0] - point_id_pos[0], 2) +
			pow(point_id[1] - point_id_pos[1], 2) +
			pow(point_id[2] + idz_2 - point_id_pos[2], 2));
		weights[5] = sqrt(// 101 =
			pow(point_id[0] + idx_2 - point_id_pos[0], 2) +
			pow(point_id[1] - point_id_pos[1], 2) +
			pow(point_id[2] + idz_2 - point_id_pos[2], 2));
		...
		for (int i = 0; i < 8; i++) {
			...
			cube_id[0] = point_id[0] + (idx_2 * (i % 2));       // x= 0,1,0,1,0,1,0,1
			cube_id[1] = point_id[1] + (idy_2 * ((i / 2) % 2)); // y= 0,0,1,1,0,0,1,1
			cube_id[2] = point_id[2] + (idz_2 * (i / 4));       // z= 0,0,0,0,1,1,1,1
			
			out += vec_wepl_cube->GetPixel(cube_id) * weights[i];
		}
...
void CbctRecon::GetAngularWEPL_SinglePoint(UShortImageType::Pointer& spUshortImage, float fAngleGap, float fAngleStart, float fAngleEnd, VEC3D calcPt, int curPtIdx, vector<WEPLData>& vOutputWEPLData, bool bAppend)
{
	...
	const double pixel_size[] = { wepl_image->GetSpacing()[0], wepl_image->GetSpacing()[1], wepl_image->GetSpacing()[2] };

	FloatImageType::RegionType region = wepl_image->GetLargestPossibleRegion();
	const size_t cube_dim[] = { region.GetSize()[0], region.GetSize()[1], region.GetSize()[2] };
    ...
    for (int i = 0; i < sizeAngles; i++)
    {
		...
		printf("%.2f, [%.2f, %.2f, %.2f]: ", curAngle, basis[0], basis[1], basis[2]);

		stArrWEPL[i].fWEPL = WEPL_from_point(isoTarget, basis, pixel_size, cube_dim,
...
#pragma omp parallel for
	for (int p = 0; p < (int)sizePOI; p++)
	{
		...
		for (int i = qRound(fAngleStart / fAngleGap); i < (sizeAngles + qRound(fAngleStart / fAngleGap)); i++)
		{
			...
			plm_long ap_idx_default[2] = { 0, 0 };
			stArrWEPL[i - qRound(fAngleStart / fAngleGap) + p * sizeAngles].fWEPL = (float)(rpl_vol->get_value(ap_idx_default, rglength)); //rgdepth
...
void CbctRecon::SLT_StopSyncFromSharedMem()
{
    ...
    while (cnt < max_cnt)
    {
        ...
        case WAIT_OBJECT_0: //0 -->This got the semaphore token
            ...
            fout << "Thread %d: wait succeeded " << GetCurrentThreadId() << std::endl;
            Sleep(10);
        ...
        case WAIT_TIMEOUT: //no need of release //258
            ...
            fout << "Thread %d: wait timed out " << GetCurrentThreadId() << std::endl;
            break;
        }
        //	ReleaseSemaphore(hSemaphore, 1, &prev_counter);
    }
...
void CbctRecon::SLT_OpenPhaseData()
{
    ...
    fin.close();
    std::cout << "NumOfPhaseData[Float]= " << phaseCnt << "  Mean Phase value= " << phaseSum / (double)phaseCnt << std::endl;
}
...
FloatImageType::Pointer CbctRecon::ConvertUshort2WeplFloat(UShortImageType::Pointer& spImgUshort)
{
	...
	typedef std::array< std::pair< float, double >, 16> double_pair_list;
	const double_pair_list lookup = { // Data from TRiP: 19990218.hlut
		std::pair<double, double>(NLMIN(float), 0),
		std::pair<double, double>(-1000.0, 0.041),
		std::pair<double, double>(-798.0, 0.244),
		std::pair<double, double>(-750.0, 0.297),
		std::pair<double, double>(-108.0, 0.943),
		std::pair<double, double>(-75.0, 0.977),
		std::pair<double, double>(0.0, 1.0),
		std::pair<double, double>(40.0, 1.042),
		std::pair<double, double>(55.0, 1.049),
		std::pair<double, double>(67.0, 1.065),
		std::pair<double, double>(262.0, 1.095),
		std::pair<double, double>(1033.0, 1.468),
		std::pair<double, double>(1432.0, 1.634),
		std::pair<double, double>(1974.0, 1.778),
		std::pair<double, double>(3000.0, 2.051),
		std::pair<double, double>(NLMAX(float), 2.051)
	};
	/*{ // plastimatch data:
		std::pair<double, double>(NLMIN(float), 0),
			std::pair<double, double>(-1000, 0.00106),
			std::pair<double, double>(0, 1.0),
			std::pair<double, double>(41.46, 1.048674),
			std::pair<double, double>(NLMAX(float), 0.005011) // wtf?
	};*/
	...
	std::function<float(float)> hu_to_dEdx = [&lookup](float val)
	{
		...
		auto lookup_upper_ptr = std::find_if(lookup.begin(), lookup.end(),
			[val](std::pair<double,  double> cur_pair) {	return val < cur_pair.first; });
		const auto lookup_upper = *lookup_upper_ptr;

		// Get the previous index:
		const auto lookup_lower = lookup[(lookup_upper_ptr - lookup.begin()) - 1]; 

		// Do linear interpolation between upper and lower data point:
		const double a = (lookup_upper.second - lookup_lower.second) / (lookup_upper.first - lookup_lower.first);
		const double b = lookup_upper.second - a * lookup_upper.first;
