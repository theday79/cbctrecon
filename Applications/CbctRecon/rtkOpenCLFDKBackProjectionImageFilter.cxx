...
namespace rtk
{
		...
		::InitDevice()
	{
		...
		im_desc.image_array_size = 1; //If you ever change the above from Image2D to Image2D Array performance will be worsened.
		...
		::GenerateData()
	{
		...
		for (unsigned int iProj = iFirstProj; iProj < (iFirstProj + nProj); iProj++)
		{
			...
			float fMatrix[12];
			for (int j = 0; j < 3; j++) { // Manual unroll 3*(one less +, 4 less %, 4 / is now * )
