...
// #include <itkRealToHalfHermitianForwardFFTImageFilter.h> // to-be-replaced with clFFT
...
namespace rtk
{
	...
	OpenCLFFTConvolutionImageFilter<TParentImageFilter>
	...
	template<class TParentImageFilter>
	typename OpenCLFFTConvolutionImageFilter<TParentImageFilter>::FFTInputImagePointer
		OpenCLFFTConvolutionImageFilter<TParentImageFilter>::PadInputImageRegion(const RegionType &inputRegion)
	{
		FloatImageType::RegionType inBuffRegion = this->GetInput()->GetBufferedRegion();
		if (inBuffRegion != this->GetInput()->GetRequestedRegion())
		{
			itkExceptionMacro(<< "OpenCLFFTConvolutionImageFilter assumes that input requested and buffered regions are equal.");
		}
		...
		itk::Image<float, 3>::Pointer paddedImage = itk::Image<float, 3>::New();
		...
		::GPUGenerateData()
	{
		...
		typename Superclass::FFTInputImageType::RegionType inputreg;
		...
		cl_float2* p_KernelFFTOpenCL = reinterpret_cast<cl_float2*>(this->m_KernelFFTOpenCL->GetBufferPointer());

		//std::cout << "Before Conv.:: Some proj value: " << paddedImage->GetBufferPointer()[1000] << std::endl;

		OpenCL_fft_convolution(inputDimension,
			kernelDimension,
			paddedImage->GetBufferPointer(),
			p_KernelFFTOpenCL
		);
		//std::cout << "After  Conv.:: Some proj value: " << paddedImage->GetBufferPointer()[1000] << std::endl;


		// CUDA Cropping and Graft Output
		typedef itk::CropImageFilter<itk::Image<float, 3U>, itk::Image<float, 3U>> CropFilter;
		...
		itk::ImageRegionConstIterator<itk::Image<float, 3U>> itS(cf->GetOutput(), this->GetOutput()->GetBufferedRegion());
		itk::ImageRegionIterator     <itk::Image<float, 3U>> itD(this->GetOutput(), this->GetOutput()->GetBufferedRegion());
		itS.GoToBegin();
		itD.GoToBegin();
		while (!itS.IsAtEnd())
		{
