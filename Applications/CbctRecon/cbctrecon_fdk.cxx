...
void CbctRecon::DoReconstructionFDK(enREGI_IMAGES target) {
  ...
  if (Tdev == CUDA_DEVT) {
    using ImageType = CUDAFloatImageType;
  }
  else {
  ...
  using castToCudaType =
    itk::CastImageFilter<FloatImageType, ImageType>;
...
// Displaced detector weighting // set pipeline //inplace filter
  ...
  if (ui.checkBox_UpdateAfterFiltering->isChecked()) {
    ...
    using CastFilterType =
      itk::CastImageFilter<ImageType, FloatImageType>;
    CastFilterType::Pointer CastFilter = CastFilterType::New();
  ...
  using StreamerType =
    itk::StreamingImageFilter<ImageType, ImageType>;
  // FDK reconstruction filtering
  ...
  feldkamp->GetRampFilter()->SetTruncationCorrection(fTruncCorFactor);
  feldkamp->GetRampFilter()->SetHannCutFrequency(fHannCut);
  feldkamp->GetRampFilter()->SetCosineCutFrequency(fCosineCut);
  feldkamp->GetRampFilter()->SetHammingFrequency(fHamming);
  feldkamp->GetRampFilter()->SetHannCutFrequencyY(fHannCutY);
  ...
  using FOVfilterType =
    rtk::FieldOfViewImageFilter<ImageType, ImageType>;
  typename FOVfilterType::Pointer FOVfilter = FOVfilterType::New();
  FOVfilter->SetGeometry(m_spCustomGeometry);
...
//                            |-1  0  0 |
  ...
  using AbsImageFilterType =
    itk::AbsImageFilter<ImageType, ImageType>;
  AbsImageFilterType::Pointer absImgFilter = AbsImageFilterType::New();
  absImgFilter->SetInput(
    flipFilter
  ...
  using CastFilterType =
    itk::CastImageFilter<ImageType, UShortImageType>;
  CastFilterType::Pointer castFilter = CastFilterType::New();
  castFilter->SetInput(multiplyImageFilter->GetOutput());
  try {
    castFilter->Update(); // YK20150109
