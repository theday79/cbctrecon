...
#
# This file was originally developed by Jean-Christophe Fillion-Robin, Kitware Inc. and
# was partially funded by NIH grant 3P41RR013218-12S1
#
# ########################################################################################
...
# .rst: .. cmake:variable:: EXTERNAL_PROJECT_DIR
#
if(NOT EXISTS "${EXTERNAL_PROJECT_DIR}")
  set(EXTERNAL_PROJECT_DIR ${CMAKE_SOURCE_DIR}/SuperBuild)
endif()

# .rst: .. cmake:variable:: EXTERNAL_PROJECT_FILE_PREFIX
#
...
# .rst: .. cmake:function:: mark_as_superbuild
#
# .. code-block:: cmake
#
...
# .. code-block:: cmake
#
# mark_as_superbuild( VARS <varname1>[:<vartype1>] [<varname2>[:<vartype2>] [...]]
# [PROJECTS <projectname> [<projectname> [...]] | ALL_PROJECTS] [LABELS <label1> [<label2>
# [...]]] )
#
# .. code-block:: cmake
#
# PROJECTS corresponds to a list of <projectname> that will be added using
# 'ExternalProject_Add' function. If not specified and called within a project file, it
# defaults to the value of 'SUPERBUILD_TOPLEVEL_PROJECT' Otherwise, it defaults to
# 'CMAKE_PROJECT_NAME'. If instead 'ALL_PROJECTS' is specified, the variables and labels
# will be passed to all projects.
#
# VARS is an expected list of variables specified as <varname>:<vartype> to pass to
# <projectname>
#
# LABELS is an optional list of label to associate with the variable names specified using
# 'VARS' and passed to the <projectname> as CMake CACHE args of the form:
...
function(mark_as_superbuild)
  ...
  if(_named_parameters_expected AND _sb_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Arguments '${_sb_UNPARSED_ARGUMENTS}' should be associated with VARS parameter !"
    )
  endif()

  if(_sb_PROJECTS AND _sb_ALL_PROJECTS)
    message(
      FATAL_ERROR "Arguments 'PROJECTS' and 'ALL_PROJECTS' are mutually exclusive !")
  endif()
  ...
  _sb_append_to_cmake_args(
    ...
    LABELS
    ${_sb_LABELS}
    ${optional_arg_ALL_PROJECTS})
endfunction()

#
# _sb_extract_varname_and_vartype(<cmake_varname_and_type> <varname_var> [<vartype_var>])
#
# <cmake_varname_and_type> corresponds to variable name and variable type passed as
# "<varname>:<vartype>"
#
...
# [<varname_var> [<vartype_var>]])
#
# <cmake_varname_and_type> corresponds to variable name and variable type passed as
# "<varname>:<vartype>"
#
# <cmake_arg_var> is a variable name that will be set to
# "-D<varname>:<vartype>=${<varname>}"
#
# <has_int_dir_var> is set to either TRUE or FALSE. FALSE means that the value does NOT
# reference ${CMAKE_CFG_INTDIR} and the generated cmake argument should be passed to
# ExternalProject_Add as CMAKE_CACHE_ARGS. TRUEmeans that the value does reference
# ${CMAKE_CFG_INTDIR} and the generated cmake argument should be passed to
...
function(
  ...
  set(${cmake_arg_var}
      -D${_varname}:${_vartype}=${_var_value}
      PARENT_SCOPE)
  set(${has_cfg_intdir_var}
      ${_has_cfg_intdir}
      PARENT_SCOPE)
...
#
# _sb_append_to_cmake_args( VARS <varname1>:<vartype1> [<varname2>:<vartype2> [...]]
# [PROJECTS <projectname> [<projectname> [...]] | ALL_PROJECTS] [LABELS <label1> [<label2>
# [...]]] )
#
# PROJECTS corresponds to a list of <projectname> that will be added using
  ...
  foreach(_sb_PROJECT ${_sb_PROJECTS})
    ...
    foreach(varname_and_vartype ${_sb_VARS})
      if(NOT TARGET ${_sb_PROJECT})
        set_property(GLOBAL APPEND PROPERTY ${_sb_PROJECT}_EP_CMAKE_ARGS
                                            ${varname_and_vartype})
        _sb_extract_varname_and_vartype(${varname_and_vartype} _varname)
      else()
        message(
          FATAL_ERROR
            "Function _sb_append_to_cmake_args not allowed because project '${_sb_PROJECT}' already added !"
        )
      endif()
    ...
    if(_sb_LABELS)
      ...
      foreach(label ${_sb_LABELS})
        set_property(GLOBAL APPEND PROPERTY ${_sb_PROJECT}_EP_LABEL_${label}
                                            ${_ep_varnames})
      endforeach()
    endif()
  endforeach()
...
function(_sb_get_external_project_arguments proj varname)
  ...
  function(_sb_collect_args proj)
    ...
    foreach(var ${_args})
      ...
      set_property(GLOBAL APPEND PROPERTY ${proj}_EP_PROPERTY_${_ep_property}
                                          ${cmake_arg})
    endforeach()
  ...
  foreach(property CMAKE_ARGS CMAKE_CACHE_ARGS)
    get_property(${proj}_EP_PROPERTY_${property} GLOBAL
                 PROPERTY ${proj}_EP_PROPERTY_${property})
    get_property(${_ALL_PROJECT_IDENTIFIER}_EP_PROPERTY_${property} GLOBAL
                 PROPERTY ${_ALL_PROJECT_IDENTIFIER}_EP_PROPERTY_${property})
    set(_all ${${proj}_EP_PROPERTY_${property}}
             ${${_ALL_PROJECT_IDENTIFIER}_EP_PROPERTY_${property}})
    if(_all)
      list(
        APPEND
        _ep_arguments
  ...
  if(CMAKE_VERSION VERSION_GREATER "3.0")
    list(
      ...
      ${_sb_CMAKE_GENERATOR_PLATFORM})
  endif()
  list(
    APPEND
    _ep_arguments
    CMAKE_GENERATOR_TOOLSET
    ${_sb_CMAKE_GENERATOR_TOOLSET})
  if(CMAKE_VERSION VERSION_GREATER "3.3")
    ...
    foreach(
      ...
      ITEMS DOWNLOAD
            ...
            TEST
            INSTALL)
      list(APPEND _ep_arguments USES_TERMINAL_${step})
    endforeach()
  endif()
  set(${varname}
      ${_ep_arguments}
...
# .rst: .. cmake:function:: ExternalProject_Message
#
# .. code-block:: cmake
#
...
#
# <output_var> is the name of CMake variable that will be set with the content of the
# stack identified by <stack_name>.
#
...
# <stack_name> corresponds to the name of stack.
#
# <output_var> is the name of CMake variable that will be set with the size of the stack
# identified by <stack_name>.
#
...
# <stack_name> corresponds to the name of stack.
#
# <item_var> names a CMake variable that will be set with the item removed from the stack
# identified by <stack_name>
#
...
# .rst: .. cmake:function:: ExternalProject_Include_Dependencies
#
# .. code-block:: cmake
#
# ExternalProject_Include_Dependencies(<project_name> [PROJECT_VAR <project_var>]
# [EP_ARGS_VAR <external_project_args_var>] [DEPENDS_VAR <depends_var>] [USE_SYSTEM_VAR
# <use_system_var>] [SUPERBUILD_VAR <superbuild_var>] [CMAKE_GENERATOR <cmake_generator>]
# [CMAKE_GENERATOR_PLATFORM <cmake_generator_platform>] [CMAKE_GENERATOR_TOOLSET
# <cmake_generator_toolset>] )
#
macro(ExternalProject_Include_Dependencies project_name)
  ...
  if(NOT _sb_PROJECT_VAR)
    ...
    # message("[${project_name}] Setting _sb_PROJECT_VAR with default value
    # '${_sb_PROJECT_VAR}'")
  endif()
  ...
  foreach(param DEPENDS EP_ARGS)
    if(NOT _sb_${param}_VAR)
      ...
      # message("[${project_name}] Setting _sb_${param}_VAR with default value
      # '${_sb_${param}_VAR}'")
    endif()
  endforeach()
  ...
  if(NOT _sb_USE_SYSTEM_VAR)
    ...
    # message("[${project_name}] Setting _sb_USE_SYSTEM_VAR with default value
    # '${_sb_USE_SYSTEM_VAR}'")
  endif()
  ...
  if(NOT _sb_SUPERBUILD_VAR)
    ...
    # message("[${project_name}] Setting _sb_SUPERBUILD_VAR with default value
    # '${_sb_SUPERBUILD_VAR}'")
  endif()

  # Set default for optional CMAKE_GENERATOR_* parameters
  foreach(varname IN ITEMS "CMAKE_GENERATOR" "CMAKE_GENERATOR_PLATFORM"
                           "CMAKE_GENERATOR_TOOLSET")
    if(NOT _sb_${varname})
      set(_sb_${varname} ${EP_${varname}})
      # message("[${project_name}] Setting _sb_${varname} with default value
      # '${_sb_${varname}}'")
    else()
  ...
  # Use system ?
  get_property(
    _use_system_set GLOBAL
    PROPERTY SB_${_sb_proj}_USE_SYSTEM
    SET)
  if(_use_system_set)
  ...
  foreach(dep ${_sb_DEPENDS})
    if(NOT
      ...
      if(_sb_USE_SYSTEM AND NOT DEFINED ${_sb_${dep}_USE_SYSTEM_VAR})
        ...
        # message(${_sb_proj} "Property SB_${dep}_USE_SYSTEM set to
        # [${_sb_USE_SYSTEM_VAR}:${_sb_USE_SYSTEM}]")
      endif()
    endif()
  ...
  foreach(dep ${_sb_DEPENDS})
    ...
    if(NOT _included)
      ...
      elseif(
        EXISTS
        "${EXTERNAL_PROJECT_ADDITIONAL_DIR}/${EXTERNAL_PROJECT_FILE_PREFIX}${dep}.cmake")
        include(
          ${EXTERNAL_PROJECT_ADDITIONAL_DIR}/${EXTERNAL_PROJECT_FILE_PREFIX}${dep}.cmake)
      else()
  ...
  # message("[${_sb_proj}] #################################") message("[${_sb_proj}]
  # Setting ${_sb_PROJECT_VAR}:${_sb_proj}") message("[${_sb_proj}] Setting
  # ${_sb_EP_ARGS_VAR}:${${_sb_EP_ARGS_VAR}}") message("[${_sb_proj}] Setting
  # ${_sb_DEPENDS_VAR}:${${_sb_DEPENDS_VAR}}") message("[${_sb_proj}] Setting
  # ${_sb_USE_SYSTEM_VAR}:${_sb_USE_SYSTEM}")
endmacro()

# .rst: .. cmake:function:: ExternalProject_Add_Empty
#
# .. code-block:: cmake
#
# ExternalProject_Add_Empty(<project_name> DEPENDS <depends> )
#
...
# .rst: .. cmake:function:: ExternalProject_Install_CMake
#
...
#
# This causes building the main project's ``install`` target to also execute the CMake
# install script for the specified external project. The project must be previously
# declared with :command:`ExternalProject_Add`.
#
...
# .rst: .. cmake:function:: ExternalProject_SetIfNotDefined
#
...
#
# The default value is set with: (1) if set, the value environment variable <var>. (2) if
# set, the value of local variable variable <var>. (3) if none of the above, the value
# passed as a parameter.
#
# Setting the optional parameter 'OBFUSCATE' will display 'OBFUSCATED' instead of the real
# value. Setting the optional parameter 'QUIET' will not display any message.
